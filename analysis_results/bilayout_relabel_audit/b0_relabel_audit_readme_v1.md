...
- `accept_bilayout_enclosed`: current Bi relabel is directly acceptable as enclosed target.
- `accept_with_minor_fix`: only tiny local fix is needed; no medium redraw; no room-scope change.
- `reject_undercoverage`: Bi is too conservative and misses camera-room content.
- `reject_ambiguous_or_oos`: boundary/OOS ambiguity prevents clean enclosed target use.
- `reject_model_error_needs_manual_fix`: the image appears to have a clear enclosed GT, but the current HoHoNet/Bi-Layout candidate requires moderate or larger manual correction before it can be used as an enclosed target. It is a manual relabel candidate, not a clean B1 target.
...
- `usable_for_B1=true` is allowed only for `accept_bilayout_enclosed`, or `accept_with_minor_fix` when the required fix is genuinely minor.
- `usable_for_B1=false` is required for `reject_undercoverage`, `reject_ambiguous_or_oos`, `reject_model_error_needs_manual_fix`, `both_wrong=true`, and unresolved `oos_suspect=true`.
- Optional future manual relabel tracking fields may include `manual_relabel_candidate`, `corrected_target_ref`, and `usable_after_manual_relabel`; they are documentation-only suggestions and are not required by the current CSV workflow.
