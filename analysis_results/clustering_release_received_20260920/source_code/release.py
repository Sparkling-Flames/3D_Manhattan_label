...
CONFIG={'release':'RC1-20260920','source_commit':'f4a6f4a3b85c08d8873c8a56066219844dc7c88b',
...
def prepare(root,new_path=None,restore_borrowed=False):
 src=root/'input/source';sys.path.insert(0,str(src))
 ...
 for key,fullrows in sorted(allby.items()):
  ...
  for m in met:
   ...
   for t in kk:
    ...
    reps={l:int(z[np.argmin(dm[np.ix_(z,z)].sum(1))]) for l in groups for z in [np.flatnonzero(lab==l)]}
