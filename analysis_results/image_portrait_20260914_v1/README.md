...
**给Pro的唯一入口：[PRO_START_HERE.md](PRO_START_HERE.md)**。算法模拟人员等假说先搁置，旧提示词暂缓。当前围绕独立真人采集与离线人员组合研究不确定性／收敛；[当前具体安排及导师对话解释](../../docs/thesis_main/第一阶段执行与人员分类续研说明_20260913.md)记录新人750份＋旧中文172份的候选名单；[独立复核](../../docs/thesis_main/全历史覆盖方案独立复核_20260916.md)记录通过项及首批165份待审核等限制，尚未派发。

本包提供648图的分类、可见特质、冻结模型表征及预测几何；历史真人快照为214图、26人、2501条canonical响应。模型覆盖不等于真人结果覆盖。人员分类保留多种候选，106张用户难易tag保留独立对照，旧实验difficulty字段停用。

已接收两轮历史粗分类及按第二轮候选更新的A—E，DINO已参加后续研究。最新发现、来源及限制见[多轮证据独立复核](../../docs/thesis_main/图片难度与收敛_多轮证据及导师讨论对照_20260916.md)；[模拟原型审读](../../docs/thesis_main/人员模拟建议独立审读与最小验证方案_20260916.md)保留独立数值核对和备选设计，不要求Pro照搬。

原始Pro报告按研究版本保存在`cloud/`，具体入口及复算缺口由PRO_START_HERE登记。早期v1/v2报告中的“未分析DINO”“尚未返回”只适用于当时；历史结果不覆盖、不冒充本轮新检验。新增假说探索的结果尚未接收。

图片审核在本地进行：[39图／44项问题入口与无图依据](review_workflow_20260915/key39/README.md)。原276图总页和39图专项页独立保存，用户审核尚待完成；云端读取问题、数值和文字，原图、权重、展示缓存及检查截图不上传。
...
6. `models/`保存每个模型的实际特征、预测及失败；原始A—E任务说明保留作来源：[A 图片特质](prompts/A.md)、[B 模型反馈](prompts/B.md)、[C 模型层](prompts/C.md)、[D 同房几何](prompts/D.md)、[E 人员分类与组合](prompts/E.md)。本轮假说探索按最新目标自主设计，不要求照旧提示词重复执行；已返回的具体目录见PRO_START_HERE。
