...
**当前续研（2026-09-15最新确认）：从历史真实作答派生难度粗分类，扩大有作答证据的研究覆盖，再分析其与648图分类、图片特质和模型表征的联系。**旧实验自带difficulty字段停用；106张用户亲填tag保留作独立对照，不用于选粗分类阈值或与新粗类混成同口径真值。直接复制[历史粗分类与图片联系的Pro续研任务](../../docs/thesis_main/历史作答难度粗分类续研任务_20260915.md)。早期k逐一比较2—7，稳定多簇允许稳定，困难候选按“当前观察内未稳定”解释；粗分类和图片分析均须实际完成，不能停在方法讨论。

106人工tag仍为简单49、中等41、困难16；另6张未定、536张没有该版选用记录。来源为`metadata/spatial_history.jsonl.gz`的`latest_selection_record.difficulty`，整理及来源核对见[人工标签对照表](difficulty_tags_20260915_v1/README.md)。仅停用旧实验字段，不按difficulty字段名误删本批用户标签。无历史或无标签的图保留648图身份，模型预测不能增加真实标签数量；本轮尚未执行新粗分类。
