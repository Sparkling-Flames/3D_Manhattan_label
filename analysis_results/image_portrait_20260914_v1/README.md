...
**给Pro只需一个入口：[PRO_START_HERE.md](PRO_START_HERE.md)**。它列明现行任务、三轮返回结果、可用输入、复算限制和下一轮输出位置；不要按文件日期猜测哪个提示词生效。2026-09-15已将主空间阅读包结果及对应代码迁入项目，原报告与审查意见分别保留。

**给Pro只需一个入口：[PRO_START_HERE.md](PRO_START_HERE.md)**。它列明现行任务、三轮返回结果、可用输入、复算限制和下一轮输出位置；不要按文件日期猜测哪个提示词生效。2026-09-15已将主空间阅读包结果及对应代码迁入项目，原报告与审查意见分别保留。

**当前续研（2026-09-15最新确认）：从历史真实作答派生难度粗分类，扩大有作答证据的研究覆盖，再分析其与648图分类、图片特质和模型表征的联系。**旧实验自带difficulty字段停用；106张用户亲填tag保留作独立对照，不用于选粗分类阈值或与新粗类混成同口径真值。直接复制[历史粗分类与图片联系的Pro续研任务](../../docs/thesis_main/历史作答难度粗分类续研任务_20260915.md)。早期k逐一比较2—8（含8），稳定多簇允许稳定，困难候选按“当前观察内未稳定”解释；粗分类和图片分析均须实际完成，不能停在方法讨论。

106人工tag仍为简单49、中等41、困难16；另6张未定、536张没有该版选用记录。来源为`metadata/spatial_history.jsonl.gz`的`latest_selection_record.difficulty`，整理及来源核对见[人工标签对照表](difficulty_tags_20260915_v1/README.md)。仅停用旧实验字段，不按difficulty字段名误删本批用户标签。无历史或无标签的图保留648图身份，模型预测不能增加真实标签数量；两轮历史粗分类研究已接入本地，尚未获得用户审核类别。

上轮106标签主空间续研见[本地逻辑审查](../../docs/thesis_main/主空间与历史粗分层逻辑复核_20260915.md)：模型收益集中性、DINO同房标签迁移与S9机制解释的核查继续有效；下一轮优先级由上述新任务接续。原报告正文及定向证据在`mainspace_logic_review_20260915/`，未迁入完整返回包或重新拟合全部模型。

**本轮直接任务（2026-09-15再次确认）：从已分类图片出发，研究图片本身的难度，以及图片特质、人员差异、不确定性与人数增长过程之间的联系。**总体主线仍是不确定性与收敛；A—E应共同回答图片哪里难、对谁难、怎样表现、能否跨图复现。模型取层是刻画图片的工具，收敛判据审计是测量保障，两者都不能取代这一图片侧问题。任务来源和Pro结果的对应价值见[本地复核开头](../../docs/thesis_main/Pro收敛续研复核_20260915.md)。

本包用于探索图片特质、冻结模型反馈与人类标注结果的关系。**五个模型均已完成648图的本地提取；DINOv3使用用户取得的Meta官方许可权重。已收回Pro首轮及收敛续研结果，独立保存在cloud/pro_exploration/；返回分析使用旧输入，尚未分析新增DINO。**648图为画像覆盖；历史真人响应仅覆盖214图、26人、2501条canonical响应。

读取返回结果前先看[2026-09-15本地复核](../../docs/thesis_main/Pro收敛续研复核_20260915.md)：文件和核心数字可核对，但轻量包缺66个NPZ缓存、KNN存在部分跨环境复现差异，收敛起点与尾段状态的窗口不能混用。原图审查仍由本地执行。
...
本地数值初查、固定案例原图核查及完整续研prompt见[DINOv3初步解释与续研提示词](DINOv3初步解释与续研提示词.md)。这份描述性分析没有使用真人结果，不是收敛预测验证。
