...
def run(root=ROOT):
 out=root/'results';out.mkdir(exist_ok=True);rows,audit=load(root);groups=collections.defaultdict(list);coverage=[]
 for r in rows:
 ...
 (out/'cache.json').write_text(json.dumps(cache,separators=(',',':')))
 ...
 assert basic['eligible']==2381 and basic['pairs']==20672 and max(errors)<1e-9
 (out/'NUMERICAL_SCOPE.json').write_text(json.dumps(basic,indent=2,default=lambda v:v.item()));print(basic)
 print(pd.DataFrame(summary).groupby('method')[['clusters','singletons']].sum().to_string())
if __name__=='__main__':
 ap=argparse.ArgumentParser();ap.add_argument('--root',type=Path,default=ROOT);args=ap.parse_args();run(args.root)
