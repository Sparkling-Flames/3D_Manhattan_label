...
def run():
 cases=json.loads((R/'results/visual_cases.json').read_text());out=[];matching=[]
 for r in cases:
 ...
 pd.DataFrame(out).to_csv(R/'results/reviewed_point_witnesses.csv',index=False)
 (R/'results/reviewed_radius_matchings.json').write_text(json.dumps(matching,ensure_ascii=False,indent=2))
