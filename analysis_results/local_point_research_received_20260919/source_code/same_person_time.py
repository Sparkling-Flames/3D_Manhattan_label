...
def run():
 ...
 for key,g in d.groupby(['image_a','image_b','context'],sort=True):
  ...
  groups.append(s)
 d.to_csv(R/'results/same_person_time_ratios.csv',index=False)
 pd.DataFrame(groups).to_csv(R/'results/same_person_time_pair_summary.csv',index=False)
 pd.DataFrame(pred).to_csv(R/'results/same_person_time_loo.csv',index=False)
