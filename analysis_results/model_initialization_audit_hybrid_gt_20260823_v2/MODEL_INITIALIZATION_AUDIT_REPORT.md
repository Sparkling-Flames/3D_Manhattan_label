# HoHoNet 模型布局初始化代理全量审计（Test 混合 GT v2）
...
|---|---:|---:|---:|---:|---:|---:|
| test | 458 | 287 | 240 | 95 | 142 | 2 |
| validation | 190 | 163 | 156 | 148 | 10 | 27 |
| all | 648 | 450 | 396 | 243 | 152 | 29 |

“严格正确”是三道门的合取；“可用初始化”是辅助运营口径，不应和论文连续指标或严格正确混为一谈。

## Test GT 修订前后敏感性

| Test GT 口径 | 拓扑错误 | 定位错误 | 几何错误 | 可用初始化 | 严格正确（含 nearly） | nearly |
|---|---:|---:|---:|---:|---:|---:|
| 全部官方 GT（旧敏感性口径） | 168 | 151 | 46 | 234 | 93 | 1 |
| 30张人工 + 428张官方（当前主口径） | 171 | 145 | 47 | 240 | 95 | 2 |

30 张人工 GT 中，`initialization_class` 相对官方敏感性口径改变 16 张，拓扑一致状态改变 13 张。混合 GT 把 Test 严格正确从 93 张调整为 95 张、可用初始化从 234 张调整为 240 张。
...
- 独立复核确认：458 个模型 TXT 与 `output/layout_json` 逐点一致，均为 ep300；Label Studio import proposal 与这些输出的图片 ID/坐标绑定无误；模型与 GT 均在 1024×512 坐标系。未发现目录、尺度、角点顺序或图片错绑制造这 171 张。
- 当前“严格失败”规则故意很严：任何多/少一个墙角对都直接失败。Test 的严格正确为 95/458，但较宽松的可用初始化为 240/458；两者回答的问题不同。
...
- 全量逐图 CSV：`analysis_results/model_initialization_audit_hybrid_gt_20260823_v2/model_initialization_metrics.csv`
- 运行清单：`analysis_results/model_initialization_audit_hybrid_gt_20260823_v2/run_manifest.json`
