...
def data(root):
 rows=[json.loads(l)for l in gzip.open(root/'inputs/responses.jsonl.gz','rt')]
 audit=pd.read_csv(root/'inputs/prior_response_audit.csv').set_index('id')
 app=s.accepted_map(root/'inputs',rows)
 rec,ad=s.prepare(rows,audit,app,key39=json.loads((root/'inputs/key39_source.json').read_text()))
 return rows,rec,ad

def run(root=R):
 out=root/'results';rows,rec,ad=data(root)
 pdf=pd.read_csv(out/'pairwise_rules.csv');mem=pd.read_csv(out/'memberships.csv')
 cache=json.loads((out/'cache.json').read_text());summ=[];each=[];details=[]
 d=json.loads((root/'inputs/user_six_review.json').read_text(encoding='utf-8-sig'))['decisions']
 cases=[]
 for e in json.loads((root/'inputs/pilot_evidence.json').read_text()):
 ...
 for e in json.loads((root/'inputs/new_six_evidence.json').read_text()):
 ...
 alt=root/'sensitivity_min_horizontal_results'
 if alt.exists():
  ...
  s.dump(out/'MIN_HORIZONTAL_CONTROL_SCOPE.json',json.loads((alt/'SCOPE.json').read_text()))
