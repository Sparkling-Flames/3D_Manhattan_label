...
def accepted_map(inp,rows):
 ...
 ev=json.loads((inp/'pilot_evidence.json').read_text());out={}
 for e in ev:
  d=dec[e['key']]
  if d['defer']:continue
  for side in ['a','b']:
...
def prepare(rows,audit,approved,association='legacy_guarded',key39=None):
 ...
 if association=='legacy_guarded':
  ...
  ns=legacy.legacy_functions(key39 if key39 is not None else json.loads((ROOT/'inputs/key39_source.json').read_text()))
 for r in rows:
...
def run(root=ROOT,association='legacy_guarded',output=None):
 ...
 rec,ad=prepare(rows,audit,approved,association,json.loads((inp/'key39_source.json').read_text()));ad.to_csv(out/'response_eligibility.csv',index=False)
 ords=[]
 for a in rec.values():
