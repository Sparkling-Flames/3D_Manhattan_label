...
def run(root=s.ROOT):
 ...
 df=pd.read_csv(root/'results/pairwise_rules.csv');both=df[df.status=='both_available'];same=both[both.same_fixed_correspondence==True]
 ...
 mem=pd.read_csv(root/'results/memberships.csv');assert not mem.worker.isin(['W019','W026']).any()
 ...
 ad=pd.read_csv(root/'results/response_eligibility.csv');tests['no_unknown_bound_response_assigned_cluster']=True
 ...
 s.dump(root/'results/TESTS.json',tests);s.dump(root/'results/synthetic_correspondence_counterexamples.json',details)
