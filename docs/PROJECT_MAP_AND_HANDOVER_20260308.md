...
- `analysis_results/`
  ...
  - `analysis_results/c_manifests_20260310/` 已补齐当前 C 线 manifest 交付包。
- `tools/`
  - 当前实验在用脚本。
- `tools/official/`
  - 正式运行入口。
...
- `analysis_results/c_manifests_20260310/trap_manifest_schema_v1.json`
  - 当前 semi trap manifest 的 schema contract。
  - 锁定 join key、status 字段、quota 分离、source_type 与 operator_id 要求。
- `analysis_results/c_manifests_20260310/natural_failure_bank_index_v1.csv`
  - 由 `trap集/复核总表_20260307.md` 结合 A 线 task registry 整理出的自然案例 bank index。
  - 每行都补了 `base_task_id` 与 task-registry 覆盖信息，避免继续只靠 basename 口头对照。
- `analysis_results/c_manifests_20260310/embedding_ood_protocol_v1.json`
  - 当前 `d_t` / `I_t_OOD` 的 frozen procedure。
  - 先冻结 embedding backend、reference pool selector、K、阈值规则与反泄漏边界，不提前伪造 realized 名单。
...
1. `trap_manifest_schema_v1` 已落盘到 `analysis_results/c_manifests_20260310/`。
2. `natural_failure_bank_index_v1` 已将已复核 semi / OOS 自然案例变成可 join 的 CSV。
3. `embedding_ood_protocol_v1` 已按当前提纲冻结为 procedure manifest。
4. `trap_manifest_draft_v1` 已对 `PreScreen_semi` 给出首版 15 行草案，并显式区分 `realized` 与 `frozen_rule`。
