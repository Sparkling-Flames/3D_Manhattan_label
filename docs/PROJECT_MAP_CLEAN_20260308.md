...
- `tools/dev_only/manhattan_ls_sandbox_panel_debug.user.js`
  - M8.1/M11 server-scoped dev-only sandbox panel debug variant for `175.178.71.217:8080`; no active_time upload, with M9 preview-only Manhattan deviation display, M10 direction-only diagnosis, sandbox meta-label guard, and official-style preview-only corner-order controls with active pair rows. It is not an official userscript, not worker-facing, performs no annotation writeback, initiates no submit, and has no routing or formal artifact integration.
- `tools/dev_only/manhattan_ls_sandbox_panel_timed.user.js`
  - M8.1/M8.3/M11 server-scoped dev-only sandbox panel timed variant for `175.178.71.217:8080`; may POST only sandbox telemetry to `/log_time` with exclusion tags and activity-gated sandbox active-time counters, with M9 preview-only Manhattan deviation display, M10 direction-only diagnosis, sandbox meta-label guard, and official-style preview-only corner-order controls with active pair rows. It is not an official userscript, not worker-facing, performs no annotation writeback, initiates no submit, and has no routing or formal artifact integration.
