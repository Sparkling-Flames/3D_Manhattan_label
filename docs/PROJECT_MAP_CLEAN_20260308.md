...
- `tools/paper_a_manhattan/`
  ...
  - `manhattan_3d_projection.py`、`run_local_3d_projection_review.py` 与 `serve_local_3d_projection_review.py`：M15.19–M15.23.4 本地投影、M15.22 candidate bridge、changed-wall/partial-warning visual diff、texture/ghost 显示控制和只读 inspection workbench；不连接云端、不写回、不进入正式 artifact。
  ...
  - `run_m1525_visual_verdict_pack.py`：将 task218_ann3741 的专家人工视觉结论及输入哈希落盘为只读 JSON/Markdown sidecar；不修改候选、投影、viewer、routing 或标注。
- `tools/paper_b/`
