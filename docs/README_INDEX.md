...
- [LOCAL_3D_REVIEW_ARTIFACT_POLICY_v1.md](paper_a_manhattan/LOCAL_3D_REVIEW_ARTIFACT_POLICY_v1.md)：A-line human-openable 3D/2D review entrypoints must live under `analysis_results/paper_a_manhattan/hypothesis_local_review/`; current focus review UI is the standard.
...
- `tools/paper_a_manhattan/run_segment_aware_manhattan_refit_3741.py` materializes the C6.5a.9/C6.5a.9.1 deterministic 12-pair wall-line intersection refit and local review preview for 3741; source pair ids are explicitly mapped to solver positions, with no random/grid perturbation, active ranking, or writeback.
- `tools/paper_a_manhattan/run_segment_refit_2d_overlay_review_3741.py` materializes the C6.5a.9.2 source-aware baseline/corrected panorama overlay for human review only; it does not create image evidence, acceptance, patches, or writeback.
- `tools/paper_a_manhattan/run_segment_aware_manhattan_refit_3741_2d_guarded.py` materializes C6.5a.9.3 deterministic pair-2/right-height guarded candidates after rejecting the old 3741 refit by human 2D review; all outputs remain review-only.
- `tools/paper_a_manhattan/run_segment_aware_manhattan_refit_3741_y_targeted.py` materializes C6.5a.9.4 deterministic baseline-x anchored y-targeted candidates for s2/s11 with optional s1 and s5/s6 semantic adjustments; no optimizer, acceptance, or writeback.
- `tools/paper_a_manhattan/run_segment_aware_manhattan_refit_3741_height_plane_preserving.py` materializes C6.5a.9.5 deterministic dominant-height-plane candidates with s2/s11 baseline-x constraints and bottom-y sensitivity diagnostics; all outputs remain human-review-only.
- `tools/paper_a_manhattan/run_segment_aware_manhattan_refit_3741_joint_xy_search.py` materializes C6.5a.9.6 task-specific bounded deterministic joint X-Y family search for 3741; it is not the C7 optimizer and does not authorize acceptance or writeback.
- `tools/paper_a_manhattan/run_m_anchor_1_3741.py` materializes M-Anchor.1.1 sidecar-driven expert-anchor constraints, id/seam mapping, per-wall residuals, footprint/height prototype cards, and a feedback ledger row; hard anchors fail closed and false visual drift is split into rejected/available rates. This is an anchor-aware audit/filter layer, not an anchor-constrained optimizer.
- M-Anchor.1.2 is schema hardening over M-Anchor.1.1 artifacts. It strengthens sidecar input validation only and does not change the emitted M-Anchor.1.1 artifact schema.
- `tools/paper_a_manhattan/run_m_anchor_3_footprint_solver.py` materializes M-Anchor.3 review-only `s6 bottom_y` sensitivity diagnostics after M-Anchor.2 human authorization; top_y/reorder/writeback/ranking/portfolio remain forbidden.
- `tools/paper_a_manhattan/run_m_anchor_3b_local_chain_footprint_solver.py` materializes M-Anchor.3b local-chain constrained footprint candidates over s5/s6/s7/s8, using hard gates before layered local residual ranking; geometry residual only decides review worthiness, while visual hard anchors and local topology remain higher priority than residual sum.
- `tools/paper_a_manhattan/run_m_anchor_3b_local_3d_review.py` bridges M-Anchor.3b top review candidates into the existing `hypothesis_local_review` 3D display style; the preview baseline is the M2-reviewed M1 footprint candidate, and all outputs remain audit-only/no-writeback.
- `tools/paper_a_manhattan/run_m_anchor_4_staged_vertical_height_solver.py` materializes M-Anchor.4 review-only candidates under the user's expanded source-pair permission set: first x/bottom_y footprint wall-direction fitting, then top_y height fitting; pair 3 stays protected and no writeback/ranking is authorized.
- `tools/paper_a_manhattan/run_m_anchor_4_1_human_guidance_bound_footprint_shadow_probe.py` materializes M-Anchor.4.1 only from the reviewed human-feedback artifact plus an explicit range sidecar: s4/s9 move left, s10 moves slightly right, other authorized pairs are micro-adjustment-only, and s6/s11/s12 stay locked. It forbids top_y and leaves M4.2 height completion blocked pending a human `partial_accept_directionally_useful` verdict.
- `tools/paper_a_manhattan/run_m_anchor_4_1_1_staged_micro_compensation_probe.py` materializes M-Anchor.4.1.1 as a deterministic ±0.15-first, fail-closed staged micro-compensation footprint probe (maximum ±0.5). It remains M4.1 audit/shadow only: top_y, height, writeback, active ranking and portfolio selection are unauthorized; floor-heading residual only determines review worthiness, never correctness or acceptance.
- `tools/paper_a_manhattan/run_m_anchor_4_1_2_visible_range_footprint_sensitivity_review.py` adds M-Anchor.4.1.2 Stage D ±1.0 visual sensitivity only; it never overrides hard anchors, is not a micro-adjustment or final refinement, and leaves M4.2 blocked. The local review renderer now displays `before/after/delta`; display-only overlays never change geometry.
- `tools/paper_a_manhattan/run_m_anchor_4_1_3_occlusion_aware_refinement.py` materializes M-Anchor.4.1.3 expert-side occlusion-aware component ablation and s10 sensitivity diagnostics. It preserves source artifacts, records explicit point-evidence supersession for s3 only, keeps top_y/read-write/M4.2 blocked, uses original panoramas for 2D overlays and compressed textures only for 3D display, and never claims candidate correctness without a human verdict.
...
- `tools/paper_a_manhattan/manhattan_verified_3d_local_assist.py`、`manhattan_local_floorprint_probe.py`、`manhattan_3d_projection.py`、`run_local_3d_projection_review.py`、`serve_local_3d_projection_review.py`、`run_manhattan_hypothesis_local_review.py`：local projection/review background tools; human-openable 3D review entrypoints are standardized under `analysis_results/paper_a_manhattan/hypothesis_local_review/` and include Focus 2D/3D review with draggable placement, pane resizing, fixed-ratio 2D display, and LS/pixel coordinate inspection.
