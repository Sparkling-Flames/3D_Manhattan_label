...
- `tools/paper_a_manhattan/run_local_3d_projection_review.py` generates M15.19–M15.23.4 local JSON/Markdown/HTML reviews, including the read-only M15.22 candidate bridge, changed-wall metadata, texture/ghost display controls, partial-diagnostic warnings, and inspection tools.
...
- `tools/paper_a_manhattan/run_m1525_visual_verdict_pack.py` records the task218_ann3741 expert visual verdict and source hashes as a read-only JSON/Markdown sidecar; it does not change candidates, projection, viewer behavior, routing, or annotations.
