...
15. [MANHATTAN_CONSTRAINED_FIT_PLAN_v1.md](MANHATTAN_CONSTRAINED_FIT_PLAN_v1.md)
   ...
   - M15.4 visual review package is generated by `tools/render_manhattan_geometry_review_sheet.py` from geometry-debug review candidate JSONL; it is offline visual review only, with local-only manual review fields and no writeback / routing / formal artifact role.
   - M15.5 manual review aggregation is generated by `tools/summarize_manhattan_geometry_manual_review.py` from the completed manual review CSV; it summarizes expert-side review and keeps M16 blocked when high-risk algorithm_overfit / uncertainty remains.
