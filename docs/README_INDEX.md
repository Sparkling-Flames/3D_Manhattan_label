...
- `tools/paper_a_manhattan/manhattan_m1520_local_candidate_search.py` provides the M15.20 bounded local candidate families, localized geometry score, collapse/self-intersection hard gates, and conservative review dispositions for task218_ann3741; it is not a global optimizer, final fix selector, annotation writer, routing input, or formal artifact.
- `tools/paper_a_manhattan/run_m1520_local_candidate_search.py` generates the M15.20 JSON and Markdown local candidate report with 2D/3D coordinates and 4-5/5-6/6-7/7-8 residual comparisons; it does not add viewer UI or mutate Label Studio data.
