...
- `tools/paper_a_manhattan/run_local_3d_projection_review.py` generates the M15.23.5 1/2/4-panel local compare grid and read-only bridges for M15.22, M15.26, and M15.27.1 candidates. Labels, texture, ghost, and camera presets are global controls; inspector, measure, and next-issue actions target only the active panel.
