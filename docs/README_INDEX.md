...
4. [../foreign_recruitment/README_EN.md](../foreign_recruitment/README_EN.md)
   - 外国招募人员专用的 HTTPS / 英文 Stage 1 标注包入口。
   - 只作为当前 P1 的隔离 onboarding 与 helper wrapper，不修改中文标注入口、frozen import JSON 或正式协议边界。

5. [PRESCREEN_STAGE1_OPERATIONAL_GUIDE_20260327.md](PRESCREEN_STAGE1_OPERATIONAL_GUIDE_20260327.md)
   - 当前 prescreen 正式运行手册。
   - 包含：正式文件清单、工具链边界、小型测试建议、运行流程。

6. [P1_PRESCREEN_LAUNCH_CHECKLIST_v1.md](P1_PRESCREEN_LAUNCH_CHECKLIST_v1.md)
   - 当前 `P1` 启动前最直接的执行清单。
   - 用于导入 smoke test、active-log 检查、`tools/audit_active_log_quality.py` 审计、导出解析检查与正式启动判据。

7. [C1_C2_ARTIFACT_FIELD_CONTRACT_v1.md](C1_C2_ARTIFACT_FIELD_CONTRACT_v1.md)
   - 当前 line B 的 `C1/C2` 工件字段合同。
   - 把 `worker_state_snapshot_C1`、`scene_coverage_gap_C1`、`task_risk_rule_manifest_v1` 等文件收口为可直接填充的 schema。

8. [RQ3_MINIMAL_EVIDENCE_CHAIN_CONTRACT_v1.md](RQ3_MINIMAL_EVIDENCE_CHAIN_CONTRACT_v1.md)
   - 当前 `RQ3` 主证据链的最小数据合同。
   - 收口 `dt` 参考库摘要、meta-label consensus sidecar、offline replay config 与 task-risk manifest。

9. [STAGE3_OOD_PREPARATION_PLAN_v1.md](STAGE3_OOD_PREPARATION_PLAN_v1.md)
...
10. [MANHATTAN_GEOMETRY_TOOL_ROADMAP_v2.md](MANHATTAN_GEOMETRY_TOOL_ROADMAP_v2.md)
...
11. [VIS_3D_GEOMETRY_COMPATIBILITY_SPEC_v1.md](VIS_3D_GEOMETRY_COMPATIBILITY_SPEC_v1.md)
...
12. [VIS_3D_GEOMETRY_FIXTURE_PLAN_v1.md](VIS_3D_GEOMETRY_FIXTURE_PLAN_v1.md)
...
13. [MANHATTAN_LS_SANDBOX_READINESS_SPEC_v1.md](MANHATTAN_LS_SANDBOX_READINESS_SPEC_v1.md)
   - M7 Label Studio sandbox readiness spec for the experiment-outside Manhattan toolchain.
   - States that actual LS operation testing starts only after an M8 dev-only sandbox prototype, and not in current formal worker-facing projects.

14. [MANHATTAN_LS_SANDBOX_OPERATION_CHECKLIST_v1.md](MANHATTAN_LS_SANDBOX_OPERATION_CHECKLIST_v1.md)
...
15. [MANHATTAN_CONSTRAINED_FIT_PLAN_v1.md](MANHATTAN_CONSTRAINED_FIT_PLAN_v1.md)
   ...
   - M15/M15.3 smoke audit and geometry-debug review outputs live under `analysis_results/manhattan_constrained_fit_smoke/`; they are dev-only sidecars, not formal round artifacts.
...
16. [PROJECT_MAP_CLEAN_20260308.md](PROJECT_MAP_CLEAN_20260308.md)
   - 当前纯净仓库地图。
   - 用来确认哪些目录、脚本、结果仍属于主链。

17. [../analysis_results/README.md](../analysis_results/README.md)
   - `analysis_results/` 根目录整理说明。
   - 用于快速区分当前主链结果目录、历史保留目录与 `legacy/` 归档区。

18. [SOP_labelstudio_experiment.md](SOP_labelstudio_experiment.md)
   - Label Studio 导入、标注、导出的一般操作 SOP。
   - 使用时应以当前 Stage 1 正式导入文件和冻结结果为准，不要反向覆盖最新 freeze。

19. [README_DEVELOPER.md](README_DEVELOPER.md)
   - 部署、日志、服务、分析工具链的开发者入口。

20. [ANALYSIS_DATA_FLOW.md](ANALYSIS_DATA_FLOW.md)
   - 上游导出、active log、quality CSV、reliability 输出之间的数据流说明。

21. [手动分析流程.md](手动分析流程.md)
   - 你自己拿到一批数据后，手动跑 Pilot / PreScreen 的分析、审计、可视化和测试的流程说明。

22. [STATISTICAL_ANALYSIS_PLAN_v1.md](STATISTICAL_ANALYSIS_PLAN_v1.md)
...
24. [AGENT_CONTEXT_INDEX.md](AGENT_CONTEXT_INDEX.md)
   - Codex 上下文路由表。
   - 按 `P1 / C1-C2 / RQ1 / RQ2 / RQ3 / Label Studio / repo map` 指向先读文件。

25. [agent_playbooks/](agent_playbooks/)
