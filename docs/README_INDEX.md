...
- [HRC_STABILIZATION_STATUS_v1.md](paper_a_manhattan/HRC_STABILIZATION_STATUS_v1.md)
...
- `tools/paper_a_manhattan/run_case_contract_fallback_audit.py` materializes the C1.1 read-only case-contract fallback audit for projection-rule-based and missing-metrics paths; it does not change contract behavior or runner selection.
- `tools/paper_a_manhattan/manhattan_candidate_source_interface.py` and `manhattan_legacy_m1528_candidate_source.py` define the C3.1 source contract and thin legacy wrapper; `run_manhattan_hypothesis_ranking_core.py` consumes M15.28 only through that wrapper, without adding candidate strategies.
- `tools/paper_a_manhattan/manhattan_constrained_v0_candidate_source.py` is the C3.3 contract-only shadow skeleton; it emits an explicit empty candidate set and has no active runner or selection role.
- `tools/paper_a_manhattan/run_constrained_v0_shadow_audit.py` materializes read-only column-x shadow JSON/Markdown audits under `analysis_results/paper_a_manhattan/constrained_v0_shadow_audit/`; it does not change the active source or ranking.
- `tools/paper_a_manhattan/run_constrained_v0_consolidation_audit.py` consolidates the two implemented constrained-v0 shadow families and their fail-closed/positive-fixture audits; it does not authorize a third family or active source replacement.
...
- `paper_a_manhattan/M15_LEGACY_ARTIFACT_DEPENDENCY_INVENTORY_v1.md` is the read-only M15.20–M15.28 dependency inventory and migration blocker; it records current fixed-path consumers and does not perform legacy migration.
