...
- `tools/paper_a_manhattan/run_local_3d_projection_review.py` generates M15.19–M15.23.5 local JSON/Markdown/HTML reviews, including the read-only M15.22 candidate bridge and the M15.23.5 1/2/4-panel compare grid. Labels, texture, ghost, and camera presets are global controls; inspector, measure, and next-issue actions target only the active panel.
