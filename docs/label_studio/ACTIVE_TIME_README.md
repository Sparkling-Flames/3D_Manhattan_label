...
- 正式标注员：安装 `tools/label_studio/official/ls_userscript_annotator.js`。
- 调试/巡检人员：安装 `tools/label_studio/official/ls_userscript_debug.js`。
...
2. 服务器端：运行日志接收服务
   ...
   - 启动服务（项目根）：
     ...
     nohup python3 tools/label_studio/cors_server.py > server.log 2>&1 &
     ```
   - 如果想把目录名改成 `logs`：
     ```bash
...
- 在本地或服务器上运行：
  ...
  python3 tools/thesis_main/analysis/lead_time_stats.py export.json --active-log active_logs --project 15 --detail
  ```
- 参数说明：
...
- 正式标注：启用 `tools/label_studio/official/ls_userscript_annotator.js`。
- 调试巡检：启用 `tools/label_studio/official/ls_userscript_debug.js`；默认不记录 active time。
...
```bash
# 在服务器上运行统计并输出到临时文件
python3 tools/thesis_main/analysis/lead_time_stats.py /home/ubuntu/export.json --active-log /home/ubuntu/workspace/HoHoNet/active_logs --project 15 --detail > /tmp/stats_project15.txt
# 然后在本地下载
