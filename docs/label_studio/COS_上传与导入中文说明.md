...
d:/Work/HOHONET/.venv/Scripts/python.exe tools/label_studio/upload_mp3d_test_to_cos.py --bucket label-images-1389474327 --region ap-guangzhou --source-dir data/mp3d_layout/test/img --key-prefix data/mp3d_layout/test/img --manifest-path analysis_results/cos_upload_manifest_prod.json
```

### B. 生成 Label Studio 导入 JSON（图片 URL 指向 COS）

```bash
d:/Work/HOHONET/.venv/Scripts/python.exe tools/label_studio/prepare_labelstudio_docker.py --layout-txt-dir data/mp3d_layout/test/label_cor --import-kind annotations --order-from-export export_label/project-2-at-2026-02-22-11-22-ee6c4607.json --image-base-url https://label-images-1389474327.cos.ap-guangzhou.myqcloud.com/data/mp3d_layout/img_v --image-ext .jpg --disable-vis3d --output-json label_studio_import_docker.json
```
...
- 当前使用：
  - `tools/label_studio/upload_mp3d_test_to_cos.py`（批量上传 COS）
  - `tools/label_studio/prepare_labelstudio_docker.py`（生成导入 JSON）
  - `tools/label_studio/official/ls_userscript_annotator.js`（正式标注员篡改猴脚本）
  - `tools/label_studio/official/ls_userscript_debug.js`（调试/巡检脚本，默认不计时）
