...
2026-09-07 当前入口：[空间标本 v3](../../analysis_results/panorama_studio_20260907_v3/index.html)。复用工作区已有的角点组排序实现并在原 12 图 / 75 版本示例包中验证发布，旧包保留。

右侧“角点组预览顺序”：先选角点，再点“所选组前移／后移”；也可输入完整排列，例如 4 个角点组输入 `4,3,1,2`，点击“应用预览”。支持完整的 1-based 或 0-based 排列，不能缺号、重复或越界。“恢复原序”精确恢复该来源原始预览。

每组包含上下两个端点，原始坐标、上下角色和 `source_pair_id` 保留；修改仅影响当前浏览页面的预览副本。重排后几何没有重新进行有效性验证，因此显示线框，禁用旧排列的约束拟合，不宣称已经修复布局。恢复原序后回到该来源原有拟合状态。切换来源或图像会重置排列，不自动保存。

“导出独立预览顺序记录”下载 JSON：包含图像与来源名称、原始身份列表、预览到原始的 0/1-based 组及端点映射、操作历史；不包含标注写回。该记录供后续核对，目前没有导入并自动重跑拟合功能。

验证：实际 Chromium 检查通过，含前后移动、完整排列、上下坐标/身份保留、精确恢复及 JSON 下载，并通过原 75 个版本的显示回归；结果见 v3 `browser_qa/QA.json`。原始计算结果未改，正式标注及协议不变。
...
### 2026-09-12：同房组历史标注入口

继续打开`analysis_results/panorama_studio_20260907_v3/index.html`，顶部输入G179等组号，选择图片和Manual／Semi条件。每簇叠加显示成员点集，可勾选人员；点击“查看3D”切换到该人员的原有3D工作台。原12个工程案例保留在案例选择器中。

当前接入260个同房展示组；既有canonical快照在648图中覆盖214图、2501条记录，无记录提示仅针对本快照。原图使用工作台既有2048px JPEG预览处理，PNG真源不变，各图按需载入。刷新命令：`.venv/Scripts/python.exe -X utf8 -m tools.thesis_main.analysis.build_room_history_studio`。

分簇复用历史q=.95结果及同一`q_partition`实现，要求q_boundary、q_wallwall均达标且点数一致；至少2名不同人员为簇，单人标法仍显示，分区不唯一时可切换候选分区。Manual和Semi各自分簇，未纳入或几何不可计算记录单列；没有把图像合成一个跨视点的簇。分簇使用已确认复核后的有效点，3D按原始导出点对顺序展示，差异在点数和来源中注明；拟合形状不用于分簇，也不是收敛或质量裁决。

`history_audit.json`记录输入快照和总量；`history_data.js`为组/图索引，`history/*.js`保存每图人员来源、原始/有效点、3D及候选分区。`history.partitions[Manual|Semi].candidates`是候选分区列表，每个成员数字索引该图`variants`，并非人员ID；未被某分区覆盖的人员必须单列显示。复核记录、原导出、正式合同均不修改。本轮13项定向测试与脚本检查通过；本轮新增交互的浏览器预览被URL安全策略阻止，不沿用下文旧版本的浏览器验证结论。
