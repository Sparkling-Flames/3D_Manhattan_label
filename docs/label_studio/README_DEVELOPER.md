...
2.  使用 `tools/label_studio/prepare_labelstudio_docker.py` 生成导入 JSON。
3.  在 Label Studio 界面通过 "Import" 上传生成的 JSON。

> 注意：Label Studio 导出时会写入 choices 的"显示文本"（`Choice value`），而不是 `alias`。
> 因此 `tools/thesis_main/analysis/analyze_quality.py` 的 `--quality_mode v2` 解析是按关键词匹配显示文本完成的。
...
python tools/thesis_main/analysis/analyze_quality.py export_pilot.json --output_dir analysis_results_pilot
```
...
```bash
python tools/thesis_main/analysis/analyze_quality.py export_main_manual.json --output_dir analysis_results
python tools/thesis_main/analysis/analyze_quality.py export_main_semi.json --output_dir analysis_results
# ... 其余 3 个数据集
```
