...
| C6 | C6 stability audit passed for shadow-only expansion；not stable ranker | C6 stability audit 结论为 `A: C6 stable enough for C3 shadow expansion`。默认 case 0017/0019 selection drift 稳定，2369/2389/gt75/task533/ordinary-compatible 以 evidence-only/regression 方式记录；C4 overstrong risk=false。仍不声明 stable ranker，仍不得输出 accepted/downstream recommendation。 |
...
C6.1 audit、Scoring Layer Contract、C4-lite、C6.2、C3.1–C3.5、C1.1 fallback audit、C1.2/C1.2.1 fail-closed contract-unavailable refactor 与 C6 stability audit 已完成；C6 audit 结论为 `A: C6 stable enough for C3 shadow expansion`。下一步允许 C3 shadow expansion only；不得接入 active runner selection，不得替换 `legacy_m1528`，不得产生 accepted/downstream recommendation。C6 仍不是 stable ranker；C7/C9/C10 继续 blocked。
...
- `tools/paper_a_manhattan/run_hrc_c6_stability_audit.py`：C6 read-only stability audit；记录 bucket selection、C4/C2/C5 driver、evidence-only regression cases 与 accepted/downstream 边界。
