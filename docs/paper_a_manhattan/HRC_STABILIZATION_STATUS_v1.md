...
| C3 | C3.1–C3.3 complete；C3.4a `column_x_alignment` shadow-only implemented；C3.4a.1 hardened after tests | column identity 必须明确 available；separation 使用 contract margin 或保守默认值；near-seam 必须 explicit safe；projection config 最小字段必须有效。不接入 runner，不产生 accepted/downstream recommendation。其余四个 family 仍 missing，`legacy_m1528` 仍是唯一 active source。 |
...
C6.1 audit、Scoring Layer Contract、C4-lite、C6.2 与 C3.1–C3.4a 已完成；`manual_post_change_audit = narrow_pass_for_selection_drift_only`。C3.4a 仅允许 `column_x_alignment` shadow artifact；不得接入 active runner selection，不得替换 `legacy_m1528`。C6 仍不是 stable ranker；C7/C9/C10 继续 blocked。
