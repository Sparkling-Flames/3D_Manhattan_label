...
| C6 | C6.3b honesty fix；multi-case bucket audit incomplete；not stable ranker | `task218_ann3741` active bucket audit passed, but 2369/2389/gt75/task533/ordinary-compatible are evidence-only / fixture-only, not active HRC bucket audits. C6 remains audit-blocked for multi-case stability and still must not output accepted/downstream recommendation. |
...
- `tools/paper_a_manhattan/run_hrc_c6_stability_audit.py`：C6.3b read-only stability audit honesty fix；区分 active HRC bucket audit 与 evidence-only / fixture-only cases，并记录 accepted/downstream 边界。
