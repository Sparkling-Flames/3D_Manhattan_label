...
- `tools/paper_a_manhattan/compute_mgeo_diagnostic.py` 是 offline audit-only MVP。
- 输入为最小 JSONL schema。
...
- `docs/paper_a_manhattan/VIS_3D_GEOMETRY_COMPATIBILITY_SPEC_v1.md`
...
`docs/paper_a_manhattan/VIS_3D_GEOMETRY_COMPATIBILITY_SPEC_v1.md` 是当前 Track C-lite / realtime assistant 的 3D preview compatibility contract，但该 Track 仍是实验外 expert-side / lab-side prototype，不是 protocol core，也不是当前 worker-facing main experiment condition。

## 6. Explicit non-goals

- 不修改 Label Studio UI。
- 不修改 `tools/label_studio/official/ls_userscript_annotator.js`。
- 不修改 `tools/label_studio/label_studio_view_config.xml`。
- 不修改 `import_json/` 或 `export_label/`。
- 不接入 `tools/thesis_main/analysis/analyze_quality.py`。
