...
当前新增 `tools/thesis_main/registry/audit_export_inventory.py`，用于把 `export_label/` 全目录冻结为可审计 source manifest，而不是继续靠口头区分 pilot 与 formal relevance。
...
python tools/thesis_main/analysis/analyze_quality.py export_label/label_studio_manual_import.json --output analysis_results/quality_report_2026.csv --dataset_group Manual_Test --analysis_role performance

# B. 追加半自动测试组 (Semi-Auto Test)
# 注意: 必须使用 --append 参数，表头会自动校验对齐
python tools/thesis_main/analysis/analyze_quality.py export_label/label_studio_semi_import.json --output analysis_results/quality_report_2026.csv --append --dataset_group SemiAuto_Test --analysis_role performance

# C. 追加校准组 (Calibration) 用于专家评分 r_u 计算
python tools/thesis_main/analysis/analyze_quality.py export_label/label_studio_calib_import.json --output analysis_results/quality_report_2026.csv --append --dataset_group Calibration_manual --analysis_role reliability
````
