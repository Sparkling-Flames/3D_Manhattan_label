...
- 小型导出样本不包含缺失的 `scope`
- `tools/thesis_main/registry/extract_truth_layer.py` 能正常解析，不会发生 join failure
- `tools/thesis_main/analysis/analyze_quality.py` 能在 smoke sample 上运行
- `OOS` 记录与主几何指标保持分离
...
建议同时跑一次：
- `python tools/thesis_main/analysis/audit_active_log_quality.py active_logs --summary-json analysis_results/.../p1_active_log_smoke_audit_summary.json --per-file-csv analysis_results/.../p1_active_log_smoke_audit.csv`
