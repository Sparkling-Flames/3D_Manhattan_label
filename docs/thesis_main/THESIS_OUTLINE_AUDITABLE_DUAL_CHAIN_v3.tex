% Paper A formal outline source
% Version: 2026-07-12
% Status: writing outline only; not a migrated manuscript.
...
\begin{quote}
本文件是 Paper A 的 LaTeX 写作载体；Markdown 版本是审计与迁移合同。正式主线固定为
\texttt{Pilot -> PreScreen -> Calibration -> Main(Test + Validation)}，轮次固定为
\texttt{P1 -> C1 -> C2 -> T1 -> V1}。本文件不回写历史预注册、protocol freeze、assignment、routing、统计执行参数或任何原始工件；当前 Overleaf 正文尚未迁移。
\end{quote}
...
\contract{四个 RQ 如何对应后文}{RQ 合同与贡献清单}{三项一级贡献}{二级创新}{把二级创新写成一级贡献}
...
\contract{哪些时间证据可进入 RQ1 primary}{owner-valid exact annotation-level browser logs}{exact active time}{known-only、task-level、lead\_time、unknown、parent-derived}{active-time 是 worker quality}
...
\contract{worker 的可解释行为维度是什么}{经过 gate 的 P1-informed 与独立后续 evidence}{越高越好的五维画像}{component/support audit}{失败率与 reliability 共用符号}
...
\contract{P1-informed profile 是否预测独立后续行为}{跨阶段独立 C1/C2/Main rows}{RQ3a predictive validity}{suspected/not-evaluable}{P1 自证或把 predictive validity 当 routing utility}
...
\contract{如何避免真实标签造成泄漏}{offline replay/shadow support 与 V1 frozen deployment logs}{offline 主比较}{V1 deployment/audit}{V1 临时并跑未注册策略}
\subsection{RQ1：效率统计}
...
\contract{P1 profile 是否解释后续独立行为}{P1→C1/C2/Main gated rows}{跨阶段 predictive estimand}{support/pending audit}{P1 自证}
\subsection{RQ3b：路由效用统计}
\contract{Full 是否优于 Random/Global}{Calibration\_manual offline replay}{固定预算策略差异}{V1 deployment、fallback、activation}{V1 作为主因果比较场}
...
\contract{画像能否预测独立后续行为}{gated P1 与 C1/C2/Main}{predictive validity}{support/pending}{P1 自证}
...
\section{结论}
\subsection{四个 RQ 的证据边界}
\contract{逐项回答 RQ1、RQ2、RQ3a、RQ3b}{第6章 primary/sensitivity/audit}{不超证据的结论}{降级规则}{将 audit 当 primary}
...
\begin{itemize}
  ...
  \item 表4：worker profile main matrix、$G_u/S_u/C_u/V_u/P_u$、raw rates、support、legacy aliases 与 freeze stage。
