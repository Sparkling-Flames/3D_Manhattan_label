...
判断只能使用 \texttt{R\_peer\_nonmultimodal}。支持多峰任务只进入 discrepancy 与
...
eligibility、active-time sentinel 和 estimator manifest 的 SHA，缺失任一依赖时只能输出
not-evaluable/candidate，不能形成正式 routing state。
