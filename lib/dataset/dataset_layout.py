...
class PanoCorBonDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Resize to target hw if specified
        if self.hw is not None:
            target_h, target_w = self.hw
            img_pil = Image.fromarray((img * 255).astype(np.uint8))
            img_pil = img_pil.resize((target_w, target_h), Image.BILINEAR)
            img = np.array(img_pil, np.float32) / 255.
            # Scale corners
            cor[:, 0] = cor[:, 0] * target_w / W
            cor[:, 1] = cor[:, 1] * target_h / H
