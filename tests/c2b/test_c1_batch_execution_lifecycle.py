...
from tools.thesis_main.analysis.run_c1_closeout_launch import bind_c2b_runtime_mapping, freeze_c1_batch
...
def _batch_scope() -> dict[str, object]:
    return {
        ...
        "w034_authorized_repair_task_ids": [f"w034-{index}" for index in range(17)],
        "w001_authorized_repair_task_ids": [f"w001-{index}" for index in range(3)],
    }


def _batch_artifacts(root: Path, *, w034_status: str = "frozen") -> None:
    root.mkdir()
    _write_csv(root / "c1_three_track_worker_state.csv", [{"worker_id": "W001", "enrollment_batch": "original", "c2_risk_model_eligible": "true"}])
    tasks = [f"w034-{index}" for index in range(17)] + [f"w001-{index}" for index in range(3)]
    _write_csv(root / "c1_row_analysis_eligibility.csv", [{"task_id": task, "base_task_id": task, "formal_assignment_eligible": "true"} for task in tasks])
    for name in (
        "analysis_dependency_manifest.json", "c1_estimand_specific_task_support.csv", "geometry_worker_task_peer_analysis.csv",
        "geometry_worker_task_loo_analysis.csv", "c1_task_adjusted_qgt_model_audit.json", "c1_structural_reliability_eb.csv",
...
def test_c1_a_snapshot_keeps_w011_completion_exception_visible(tmp_path: Path) -> None:
    ...
    result = freeze_c1_batch(type("Args", (), {"c1_output_dir": c1_dir, "batch_scope_manifest": scope, "output": tmp_path / "snapshot.json"})())
    assert result["status"] == "provisional"
    assert "repair_or_completion_exception_missing:w011-missing" in result["blockers"]
