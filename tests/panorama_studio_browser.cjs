...
(async()=>{
  ...
  try{
    ...
    const orderBefore=await page.evaluate(()=>STUDIO.snapshot());
    const firstPair=JSON.parse(orderBefore.sourceGeometry).pairs[0];
    await page.locator('#order-later').click();
    const reordered=await page.evaluate(()=>STUDIO.snapshot());
    assert.deepEqual(reordered.previewOrder.slice(0,2),[1,0]);
    assert.equal(reordered.fitStatus,'blocked');
    assert.equal(JSON.parse(reordered.geometry).pairs[1].source_pair_id,firstPair.source_pair_id);
    assert.deepEqual(JSON.parse(reordered.geometry).pairs[1].top,firstPair.top);
    assert.deepEqual(JSON.parse(reordered.geometry).pairs[1].bottom,firstPair.bottom);
    assert.equal(reordered.sourceGeometry,orderBefore.sourceGeometry);
    assert.equal(reordered.orderRecord.preview_position_to_original_zero_based[1],0);
    assert.equal(reordered.orderRecord.preview_position_to_original_one_based[1],1);
    assert.equal(reordered.orderRecord.top_bottom_roles_preserved,true);
    const downloadPromise=page.waitForEvent('download');await page.locator('#order-export').click();
    assert.ok((await downloadPromise).suggestedFilename().endsWith('_preview_order.json'));
    await page.locator('#order-restore').click();
    const restored=await page.evaluate(()=>STUDIO.snapshot());
    assert.deepEqual(restored.previewOrder,restored.previewOrder.map((_,i)=>i));
    assert.equal(restored.geometry,restored.sourceGeometry);
    await page.locator('#order-permutation').fill(restored.previewOrder.map((_,i)=>i).reverse().join(','));
    await page.locator('#order-apply').click();
    const zeroBased=await page.evaluate(()=>STUDIO.snapshot());
    assert.deepEqual(zeroBased.previewOrder,[...zeroBased.previewOrder].sort((a,b)=>b-a));
    assert.equal(zeroBased.fitStatus,'blocked');
    await page.locator('#order-restore').click();
    checks.push('paired-group ordering preserves source IDs and top/bottom roles; restore is exact');
