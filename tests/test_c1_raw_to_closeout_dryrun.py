...
def test_raw_to_finalize_contract_positive_path_uses_only_materialized_intermediates(tmp_path: Path) -> None:
    ...
    _csv(score, score_fields, [{field: (row.get(field, "") if field in {"project_id", "ls_runtime_task_id", "worker_id", "annotation_id", "response_hash", "source_export_sha256"} else "iou_to_consensus_loo" if field == "r_u_metric_name" else "0.9" if field == "r_u_metric_value" else "higher_is_better" if field == "r_u_metric_direction" else "identity_0_1" if field == "r_u_normalization_rule" else "valid" if field == "r_u_score_status" else "synthetic_external_score") for field in score_fields} for row in quality_rows])
