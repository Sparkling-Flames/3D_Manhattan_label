...
def test_geometry_materializer_emits_candidate_sidecars_with_common_fields(tmp_path: Path) -> None:
    ...
    geometry.write_text("\n".join(json.dumps({"task_id": "t1", "worker_id": worker, "condition": "manual", "pool": "Calibration_core", "corners_px": [[100, 100 + offset], [100, 400], [500, 100 + offset], [500, 400]]}) for worker, offset in [("w1", 0), ("w2", 1), ("w3", 2)]) + "\n", encoding="utf-8")
