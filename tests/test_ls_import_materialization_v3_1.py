...
def test_ls_import_materialization_audit(tmp_path: Path) -> None:
    ...
    mapping_rows = []
    for group, prefix, n in [("Calibration_anchor", "a", 12), ("Calibration_core", "c", 75), ("Calibration_semi", "s", 25), ("Calibration_reserve", "r", 13)]:
        for i in range(n):
            mapping_rows.append({"task_id": f"{prefix}{i}", "base_task_id": f"{prefix}{i}", "inner_id": str(len(mapping_rows) + 1), "task_url": "http://task", "intended_project_group": group})
    _csv(out / "ls_project_mapping_audit_v3_1.csv", ["task_id", "base_task_id", "inner_id", "task_url", "intended_project_group"], mapping_rows)
    internal = []
