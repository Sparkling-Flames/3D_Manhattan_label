...
def test_c6_stability_audit_regression_and_evidence_only_cases():
    ...
    assert payload["audit_conclusion"] == "A: C6 stable enough for C3 shadow expansion"

    dense = payload["cases"]["task218_ann2369"]
    assert dense["audit_mode"] == "regression_evidence_only"
    assert dense["dense_but_distinct_not_collapsed"] is True
    ...
    assert height["height_dominant_not_suppressed_by_direction_or_plane_proxy"] is True
    ...
    assert task533["pair_merge_or_duplicate_recommendation"] is False

    compatible = payload["cases"]["ordinary_compatible"]
    assert compatible["fixture_present"] is True
    assert compatible["meaningless_candidate_preference_detected"] is False
