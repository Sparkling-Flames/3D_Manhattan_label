...
if(window.STUDIO_HISTORY){
  ...
  function render(){const c=dataset.cases[currentCase];if(!c.history)return;
    ...
    $('history-clusters').replaceChildren(...sets.map(({members,title})=>{
      ...
      for(const [text,checked] of [['全选',true],['全不选',false]]){const button=document.createElement('button');button.textContent=text;button.onclick=()=>{selected.clear();if(checked)members.forEach(i=>selected.add(i));buttons.querySelectorAll('input[type=checkbox]').forEach(input=>input.checked=checked);draw();};h.append(button);}
