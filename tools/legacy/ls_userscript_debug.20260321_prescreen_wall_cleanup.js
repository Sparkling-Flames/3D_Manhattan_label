...
(function () {
  ...
  const PREVIEW_PANEL_PAIR_JUMP_ID = "hohonet-preview-pair-jump";
  ...
  // 当前主流程以 Corner 为准。Wall polygon 保留给后续语义分割，
  // 默认不再作为 3D 预览的回退输入，避免密集曲线点把预览配对拖歪。
  // 如确需恢复旧行为，可在控制台开启：
  //   localStorage.setItem('HOHONET_ENABLE_POLYGON_FALLBACK_3D', '1')
  function isPolygonFallbackEnabled() {
    try {
      return (
        window.localStorage.getItem("HOHONET_ENABLE_POLYGON_FALLBACK_3D") === "1"
      );
    } catch (e) {
      return false;
    }
  ...
  const SCRIPT_VERSION = "0.24-debug";
  ...
  console.log(
    "HoHoNet polygon fallback: disabled by default; enable with localStorage.HOHONET_ENABLE_POLYGON_FALLBACK_3D=1",
  );
  ...
  function getPreviewControlPanelElements() {
    return {
      ...
      jumpBtn: document.getElementById(PREVIEW_PANEL_PAIR_JUMP_ID),
...
`;
  ...
  function updatePreviewControlPanelUi() {
    ...
    if (controls.jumpBtn) controls.jumpBtn.disabled = !hasData;
  ...
  function ensurePreviewControlPanel(anchorEl = null) {
      ...
      <div id="${PREVIEW_PANEL_BODY_ID}">
        ...
        <div class="hp-section">
          ...
          <div class="hp-row">
            ...
            <span>对</span>
            <button id="${PREVIEW_PANEL_PAIR_JUMP_ID}" type="button">跳转</button>
          </div>
          <div class="hp-row">
    ...
    const runJumpFromPairInput = () => {
      currentPreviewInputDraft.pairDirty = false;
      postPreviewOrderCommand("jump_pair", {
    ...
    document
      .getElementById(PREVIEW_PANEL_PAIR_JUMP_ID)
      .addEventListener("click", runJumpFromPairInput);
    pairInput?.addEventListener("keydown", (event) => {
      if (event.key === "Enter") {
        runJumpFromPairInput();
      }
    });
    pairInput?.addEventListener("change", runJumpFromPairInput);
    document
      ...
      if (!btn) {
        ...
        btn.onclick = function () {
          ...
          if (keypoints.length > 0) {
            ...
            if (!isPolygonFallbackEnabled()) {
              alert(
                "未找到 Corner 点。当前版本默认不再使用 Wall 多边形回退生成 3D 预览，以避免密集墙面点导致错误配对。\n\n请优先标注 Corner 点；如确需恢复旧行为，可在控制台执行 localStorage.setItem('HOHONET_ENABLE_POLYGON_FALLBACK_3D', '1')。",
              );
              return;
            }
          ...
          const storedOverride = loadPreviewOrderOverride(previewTaskKey);
