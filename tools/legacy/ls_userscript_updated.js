...
// @version      0.22
// @description  连接 Label Studio 与 HoHoNet 3D 查看器 (v0.22: 默认关闭右下角调试面板)
// @author       HoHoNet
...
(function () {
  ...
  // active_time 默认记录；调试/巡检人员可在本地显式关闭：
  //   localStorage.setItem('HOHONET_DISABLE_ACTIVE_TIME', '1')
  // 恢复记录：
  //   localStorage.removeItem('HOHONET_DISABLE_ACTIVE_TIME')
  function isActiveTimeLoggingDisabled() {
    try {
      return window.localStorage.getItem("HOHONET_DISABLE_ACTIVE_TIME") === "1";
    } catch (e) {
      return false;
    }
  }
  ...
  const HOHONET_VIS_3D_URL = (sessionId) =>
    `${getHelperBaseUrl()}/tools/vis_3d.html?v=${sessionId}`;
  ...
  console.log(
    "HoHoNet active_time switch: disable with localStorage.HOHONET_DISABLE_ACTIVE_TIME=1",
  );
  ...
  function normalizeChoiceToken(raw) {
    ...
    if (l === "trivial" || l.includes("(trivial)") || s.includes("非常简单")) return "trivial";
    if (l === "acceptable" || l.includes("acceptable") || s.includes("模型标注质量好")) return "acceptable";
  ...
  function matchesFieldName(actual, expected) {
    ...
    if (!a || !e) return false;
    if (a === e) return true;
    if (a.endsWith(`.${e}`) || a.endsWith(`:${e}`) || a.endsWith(`/${e}`)) return true;
  ...
  function isAcceptableToken(token) {
    const t = String(token || "").trim().toLowerCase();
    return t === "acceptable" || t.includes("acceptable") || t.includes("模型标注质量好");
  }

  function isMetaGuardDebugEnabled() {
    try {
      return window.localStorage.getItem("HOHONET_META_GUARD_DEBUG") === "1";
    } catch (e) {
      return false;
    }
  }
  ...
  function collectFromDomContainer(container) {
    ...
    const ariaChecked = container.querySelectorAll("[role='checkbox'][aria-checked='true'], [role='radio'][aria-checked='true']");
  ...
  function validateMetaChoices(store) {
    ...
    const modelIssue = getSelectedChoicesByField(store, "model_issue");
    const condition = getTaskCondition(store).toLowerCase();
    metaGuardDebug("validateMetaChoices", { difficulty, modelIssue, condition });
    ...
    const modelIssueRequired = condition.includes("semi");

    // 临时调试 alert
    if (window.localStorage.getItem("HOHONET_META_GUARD_DEBUG") === "1") {
      alert(`DEBUG: difficulty=${JSON.stringify(difficulty)}, hasTrivial=${hasTrivial}, hasNonTrivial=${hasNonTrivial}\nmodelIssue=${JSON.stringify(modelIssue)}, hasAcceptable=${hasAcceptable}, hasNonAcceptable=${hasNonAcceptable}`);
    }

    if (difficulty.length === 0) {
      errors.push("Difficulty 不能为空（至少选择 1 项）");
    }
    if (hasTrivial && hasNonTrivial) {
      errors.push("Difficulty 冲突：trivial 不能与其他困难标签共存");
    }
    if (modelIssueRequired && modelIssue.length === 0) {
      errors.push("Model Issue 不能为空（semi 条件下至少选择 1 项）");
    }
    if (hasAcceptable && hasNonAcceptable) {
  ...
  function shouldGuardAction(target) {
    ...
    const keys = ["submit", "完成", "提交", "save", "保存", "next", "下一条"];
  ...
  function installMetaSubmitGuard() {
    ...
    document.addEventListener(
      ...
      (event) => {
        if (!isGuardEnabled() || !isLikelyAnnotationPage()) return;
        const isSubmitHotkey = (event.ctrlKey || event.metaKey) && event.key === "Enter";
    ...
    console.log("HoHoNet Meta Guard: enabled (set localStorage.HOHONET_STRICT_META_GUARD=0 to disable)");
  }
          ...
          for (let i = 0; i < points.length; i++) {
            ...
            if (bestJ !== -1) {
              ...
              paired.push({
                ...
                y_ceiling: Math.min(points[i].y, points[j].y),
                y_floor: Math.max(points[i].y, points[j].y),
                originalPoints: [points[i], points[j]],
              });
            }
          }
  ...
  async function flushActiveTime(
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(`[FLUSH] 上报异常: ${response.status} ${response.statusText}`);
