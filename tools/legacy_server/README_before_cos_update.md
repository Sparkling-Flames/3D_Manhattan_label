...
- `tools/`：当前实验在用脚本（split、导入准备、分析、汇总、可视化、运维辅助）。
- `tools/legacy/research_prototypes/`：阶段性原型或历史脚本（不作为当前主流程入口）。

若需复现实验，以 `tools/` 根目录脚本和 `docs/README_INDEX.md` 的“当前必读”为准。
...
浏览器助手位于 `tools/ls_userscript.js` (Tampermonkey)。它注入 3D 查看器并定期发送 POST `/log_time`。
