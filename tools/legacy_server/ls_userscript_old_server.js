...
// @version      0.21
// @description  连接 Label Studio 与 HoHoNet 3D 查看器 (v0.21 修复: 计时精度与审计字段)
// @author       HoHoNet
// @match        http://106.53.106.49:8080/*
// @match        https://106.53.106.49:8080/*
// @grant        none
// ==/UserScript==
...
(function () {
  ...
  // ---- Deployment config (avoid hard-coded IP/port) ----
  // Set via browser console if needed:
  //   localStorage.setItem('HOHONET_HELPER_BASE_URL', 'http://106.53.106.49:8000');
  // or to reuse same origin (if you reverse-proxy /tools + /log_time under Label Studio):
  //   localStorage.setItem('HOHONET_HELPER_BASE_URL', location.origin);
  const HOHONET_HELPER_BASE_URL =
    window.localStorage.getItem("HOHONET_HELPER_BASE_URL") ||
    "http://106.53.106.49:8000";

  const HOHONET_VIS_3D_URL = (sessionId) =>
    `${HOHONET_HELPER_BASE_URL}/tools/vis_3d.html?v=${sessionId}`;
  const HOHONET_LOG_TIME_URL = `${HOHONET_HELPER_BASE_URL}/log_time`;
  const HOHONET_ASSET_URL = (filename) =>
    `${HOHONET_HELPER_BASE_URL}/assets/${filename}`;

  // Optional: protect /log_time from public abuse.
  // Set once in browser console:
  //   localStorage.setItem('HOHONET_LOG_TOKEN', '<your-secret>')
  const HOHONET_LOG_TOKEN =
    window.localStorage.getItem("HOHONET_LOG_TOKEN") || "";
  ...
  function rewriteTextureUrlForViewer(originalUrl) {
    ...
    try {
      const helperBase = new URL(HOHONET_HELPER_BASE_URL, window.location.href);
    ...
    try {
      const response = await fetch(HOHONET_LOG_TIME_URL, {
        ...
        headers: {
          ...
          ...(HOHONET_LOG_TOKEN
            ? { "X-HOHONET-TOKEN": HOHONET_LOG_TOKEN }
            : {}),
        },
        body: JSON.stringify({
