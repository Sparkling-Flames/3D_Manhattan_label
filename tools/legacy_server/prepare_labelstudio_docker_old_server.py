...
# 配置
# 假设我们在 d:\Work\HOHONET 下运行 python tools/cors_server.py
# 因为浏览器是在宿主机(Ubuntu)上运行，直接访问 localhost:8000 即可
# 注意：如果是多人协作，必须将 localhost 替换为宿主机的局域网 IP (例如 10.252.24.80)
# 迁移到云服务器：使用公网 IP
# 可通过环境变量 HOHONET_BASE_URL 覆盖，例如：
#   HOHONET_BASE_URL="http://106.53.106.49:8000"
#   HOHONET_BASE_URL="http://106.53.106.49"        (若 nginx 监听 80)
...
def create_import_json():
    ...
    for idx, txt_file in enumerate(txt_files):
        try:
            ...
            iframe_url = f"{BASE_URL}/tools/vis_3d.html?w={W}&h={H}&data={encoded_data}"

            # 获取图片文件名
            # txt 文件名: ID_hash.txt -> 图片文件名: ID_hash.png
            basename = os.path.basename(txt_file).replace('.txt', '.png')
            img_url = f"{BASE_URL}/{IMAGE_DIR_REL}/{basename}"
