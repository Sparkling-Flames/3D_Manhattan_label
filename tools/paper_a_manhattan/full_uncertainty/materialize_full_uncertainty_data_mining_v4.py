"""Comprehensive advisor-facing annotation data audit, version 4.
...
DEFAULT_OUTPUT = ROOT / "analysis_results" / "full_uncertainty_data_mining_20260821_v4"
SEED = 20260821
...
def load_data() -> dict[str, pd.DataFrame]:
    data = {
        ...
        "difficulty_proxy": read_csv(C1_MINING / "DIFFICULTY_PROXY_COVERAGE.csv"),
        ...
        "raw_active_events": read_csv(PACKAGE / "raw_active_event_fact.csv"),
...
def attach_authoritative_analysis_status(unified: pd.DataFrame, row_inclusion: pd.DataFrame) -> pd.DataFrame:
    """Attach stage-authoritative assignment status and observed Semi pairing."""
    frame = unified.copy()
    frame["canonical_annotation_id"] = frame.get("canonical_annotation_id", pd.Series([""] * len(frame))).map(clean)
    frame["formal_assignment_status"] = "not_recorded"
    frame["formal_assignment_status_source"] = ""
    frame["primary_eligibility_status"] = "not_recorded"
    frame["primary_eligibility_status_source"] = ""

    p1 = frame["stage"].eq("P1")
    frame.loc[p1, "formal_assignment_status"] = "not_applicable"
    frame.loc[p1, "formal_assignment_status_source"] = "P1 uses eligible_for_primary_analysis, not the formal-assignment gate"
    p1_source_path = ROOT / "analysis_results/prescreen_closeout_final_gold_v2_20260701/prescreen_canonical_annotations.csv"
    p1_source = read_csv(p1_source_path)
    p1_source["canonical_annotation_id"] = p1_source["canonical_annotation_id"].map(clean)
    p1_lookup = p1_source.drop_duplicates("canonical_annotation_id").set_index("canonical_annotation_id")["eligible_for_primary_analysis"]
    p1_values = frame.loc[p1, "canonical_annotation_id"].map(p1_lookup)
    p1_recorded = p1_values.map(clean).ne("")
    frame.loc[p1 & frame.index.isin(p1_values.index[p1_recorded]), "primary_eligibility_status"] = p1_values.loc[p1_recorded].map(
        lambda value: "eligible" if truth(value) else "ineligible"
    )
    frame.loc[p1, "primary_eligibility_status_source"] = f"{p1_source_path.relative_to(ROOT).as_posix()}:eligible_for_primary_analysis"

    c1_lookup = row_inclusion.copy()
    if not c1_lookup.empty:
        c1_lookup["canonical_annotation_id"] = c1_lookup["canonical_annotation_id"].map(clean)
        c1_status = c1_lookup.drop_duplicates("canonical_annotation_id").set_index("canonical_annotation_id")["formal_assignment_eligible"]
        mask = frame["stage"].eq("C1")
        values = frame.loc[mask, "canonical_annotation_id"].map(c1_status)
        recorded = values.map(clean).ne("")
        frame.loc[mask & frame.index.isin(values.index[recorded]), "formal_assignment_status"] = values.loc[recorded].map(
            lambda value: "eligible" if truth(value) else "ineligible"
        )
        frame.loc[mask, "formal_assignment_status_source"] = (
            "analysis_results/annotation_uncertainty_manual_semi_20260820_v2/ROW_INCLUSION_CLASSIFICATION.csv:formal_assignment_eligible"
        )
    ...
    for stage, path in c2_sources.items():
        source = read_csv(path)
        source["canonical_annotation_id"] = source["canonical_annotation_id"].map(clean)
        lookup = source.drop_duplicates("canonical_annotation_id").set_index("canonical_annotation_id")["formal_assignment_eligible"]
        mask = frame["stage"].eq(stage)
        values = frame.loc[mask, "canonical_annotation_id"].map(lookup)
        recorded = values.map(clean).ne("")
        frame.loc[mask & frame.index.isin(values.index[recorded]), "formal_assignment_status"] = values.loc[recorded].map(
            lambda value: "eligible" if truth(value) else "ineligible"
        )
        frame.loc[mask, "formal_assignment_status_source"] = f"{path.relative_to(ROOT).as_posix()}:formal_assignment_eligible"

    frame["design_has_semi_arm"] = frame["stage"].eq("C1")
    frame["task_has_semi_candidate"] = False
    if not c1_lookup.empty:
        semi_lookup = c1_lookup.drop_duplicates("canonical_annotation_id").set_index("canonical_annotation_id")["has_semi_candidate"]
        c1_mask = frame["stage"].eq("C1")
        semi_values = frame.loc[c1_mask, "canonical_annotation_id"].map(semi_lookup)
        frame.loc[c1_mask, "task_has_semi_candidate"] = semi_values.map(truth).to_numpy()
    frame["semi_pairing_status"] = np.select(
        [
            ~frame["stage"].eq("C1"),
            frame["condition"].eq("semi"),
            frame["condition"].eq("manual") & frame["task_has_semi_candidate"],
            frame["condition"].eq("manual") & ~frame["task_has_semi_candidate"],
        ],
        ["not_applicable_outside_c1_paired_design", "semi_candidate", "manual_paired_task", "manual_only_no_semi_candidate"],
        default="not_recorded",
    )
    return frame


def orthogonal_record_flags(unified: pd.DataFrame, row_inclusion: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    frame = attach_authoritative_analysis_status(unified, row_inclusion)
    ...
    frame["formal_assignment_eligible_bool"] = frame["formal_assignment_status"].eq("eligible")
    ...
    definitions: list[tuple[str, str, pd.Series]] = [
        ...
        ("formal_assignment_ineligible", "正式分配资格不足", frame["formal_assignment_status"].eq("ineligible")),
        ...
        ("manual_only_no_semi_candidate", "只有 Manual、没有 Semi 候选", frame["semi_pairing_status"].eq("manual_only_no_semi_candidate")),
    ...
    for code, label, _mask in definitions:
        ...
        summary_rows.append({
            ...
            "formal_assignment_eligible_count": int(subset["formal_assignment_eligible_bool"].sum()),
...
def raw_record_ledger(raw: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Keep every raw annotation and crosswalk non-selected versions by context."""
    frame = raw.copy()
    for column in ("stage", "project_id", "ls_runtime_task_id", "base_task_id", "worker_id", "annotation_id", "canonical_annotation_id", "canonical_join_status"):
        if column not in frame:
            frame[column] = ""
    frame["stage"] = frame["stage"].map(normalise_stage)
    frame["worker_id"] = frame["worker_id"].map(worker_id)
    frame["annotation_id"] = frame["annotation_id"].map(clean)
    frame["raw_record_role"] = np.where(frame["canonical_join_status"].eq("matched"), "selected_canonical_context", "raw_only_auditable_version")
    keys = ["stage", "project_id", "ls_runtime_task_id", "base_task_id", "worker_id"]
    selected = frame[frame["canonical_join_status"].eq("matched")][keys + ["annotation_id", "canonical_annotation_id"]].copy()
    selected = selected.rename(columns={"annotation_id": "selected_annotation_id", "canonical_annotation_id": "selected_canonical_annotation_id"})
    selected = selected.groupby(keys, dropna=False, as_index=False).agg(
        selected_annotation_ids=("selected_annotation_id", lambda values: ";".join(sorted(set(filter(None, map(clean, values)))))),
        selected_canonical_annotation_ids=("selected_canonical_annotation_id", lambda values: ";".join(sorted(set(filter(None, map(clean, values))))))
    )
    frame = frame.merge(selected, how="left", on=keys, validate="many_to_one")
    frame["crosswalk_relation"] = np.where(
        frame["canonical_join_status"].eq("matched"),
        "self_selected_record",
        np.where(frame["selected_annotation_ids"].fillna("").ne(""), "crosswalk_to_selected_context", "selected_context_not_found"),
    )
    frame["crosswalk_boundary_zh"] = "上下文映射不声明版本先后；selected 记录可能早于、晚于或位于 raw-only 记录之间。"
    raw_only = frame[frame["raw_record_role"].eq("raw_only_auditable_version")].copy()
    return frame, raw_only


def time_measurement_audit(flagged: pd.DataFrame, raw_events: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Separate frozen active time, missing active time and excluded lead-time proxies."""
    frame = flagged.copy()
    versions_by_key: dict[tuple[str, str, str, str], set[str]] = defaultdict(set)
    versions_by_context: dict[tuple[str, str, str], set[str]] = defaultdict(set)
    versions_by_source: dict[str, set[str]] = defaultdict(set)
    for record in raw_events.itertuples(index=False):
        payload = parse_jsonish(getattr(record, "raw_event_json", "")) or {}
        version = clean(payload.get("script_version")) if isinstance(payload, dict) else ""
        key = tuple(scalar_id(value) for value in (
            getattr(record, "project_id", ""), getattr(record, "task_id", ""),
            worker_id(getattr(record, "annotator_id", "")), getattr(record, "annotation_id", ""),
        ))
        if version:
            versions_by_key[key].add(version)
            versions_by_context[key[:3]].add(version)
            versions_by_source[Path(clean(getattr(record, "source_path", ""))).name].add(version)
    runtime_ids = frame.get("runtime_task_id", pd.Series([""] * len(frame), index=frame.index))
    task_ids = frame.get("task_id", runtime_ids)
    event_task_ids = np.where(frame["stage"].eq("C1") & runtime_ids.map(clean).ne(""), runtime_ids, task_ids)
    version_rows: list[str] = []
    source_files_series = frame.get("active_time_source_file", pd.Series([""] * len(frame), index=frame.index))
    for project, task, worker, annotation, source_files in zip(
        frame["project_id"], event_task_ids, frame["worker_id"], frame["annotation_id"], source_files_series
    ):
        key = tuple(scalar_id(value) for value in (project, task, worker, annotation))
        versions = set(versions_by_key.get(key, set()))
        for source_file in clean(source_files).split(";"):
            versions.update(versions_by_source.get(Path(source_file).name, set()))
        if not versions and clean(task):
            versions.update(versions_by_context.get(key[:3], set()))
        version_rows.append(";".join(sorted(versions)) or "not_recorded")
    frame["active_time_script_versions"] = version_rows
    frame["time_measurement_lane"] = np.select(
        [
            frame["stage"].eq("C1") & frame["active_time_measurement_class"].eq("formal_frozen"),
            frame["stage"].eq("C1") & frame["active_time_measurement_class"].eq("missing"),
            ~frame["stage"].eq("C1") & frame["active_time_measurement_class"].eq("formal_frozen"),
            frame["active_time_measurement_class"].eq("lead_time_proxy_excluded"),
        ],
        ["c1_formal_active_log", "c1_active_time_missing", "other_stage_formal_active_log", "lead_time_proxy_excluded"],
        default="unclassified",
    )
    frame["lead_time_is_active_time"] = False
    summary = frame.groupby("time_measurement_lane", dropna=False).agg(
        record_count=("annotation_id", "size"), active_time_observed_count=("active_time_observed_seconds", "count"),
        lead_time_observed_count=("lead_time_seconds", "count"),
        script_versions=("active_time_script_versions", lambda values: ";".join(sorted({version for item in values for version in clean(item).split(";") if version}))),
    ).reset_index()
    summary["interpretation_zh"] = np.where(
        summary["time_measurement_lane"].eq("lead_time_proxy_excluded"),
        "仅有 Label Studio 经过时间；不得进入 Active time 汇总。",
        "Active time 按冻结来源保留；脚本版本未记录时明确标为 not_recorded。",
    )
...
def semi_convergence_expansion(data: Mapping[str, pd.DataFrame], image_map: Mapping[str, str]) -> pd.DataFrame:
    ...
    target["exact_direction"] = np.select(
    ...
    target["uncertainty_direction"] = np.select(
        [target["delta_shannon_entropy"].lt(-0.05), target["delta_shannon_entropy"].gt(0.05)],
        ["semi_convergence", "semi_expansion"], default="near_zero_change",
    )
    target["direction_tolerance_010"] = np.select(
        [target["delta_shannon_entropy"].lt(-0.10), target["delta_shannon_entropy"].gt(0.10)],
        ["semi_convergence", "semi_expansion"], default="near_zero_change",
    )
    target["magnitude_class_fixed"] = np.select(
        [abs_delta.le(0.05), abs_delta.le(0.10)],
        ["near_zero_le_0_05", "small_gt_0_05_le_0_10"], default="larger_gt_0_10",
    )
    q1, q2 = abs_delta.quantile([1 / 3, 2 / 3])
    target["relative_magnitude_tertile"] = pd.cut(
        abs_delta, bins=[-np.inf, q1, q2, np.inf], labels=["lower_third", "middle_third", "upper_third"], include_lowest=True
    ).astype(str)
    target["magnitude_band"] = target["magnitude_class_fixed"]
    ...
    target["interpretation_zh"] = target.apply(
        lambda row: (
            ...
            "Manual 与 Semi 的任务级熵差处于预定义 ±0.05 容差内。"
        ), axis=1
    )
...
def building_bootstrap_interval(frame: pd.DataFrame, estimator: Any, *, replicates: int = 10_000, seed: int = SEED) -> tuple[float, float]:
    groups = [group.reset_index(drop=True) for _, group in frame.groupby("building_id", sort=True)]
    if not groups:
...
def exact_building_sign_flip(frame: pd.DataFrame, value_column: str) -> float:
    grouped = frame.groupby("building_id")[value_column].agg(["mean", "size"])
    observed = abs(float(np.average(grouped["mean"], weights=grouped["size"])))
    exceed = 0
    total = 2 ** len(grouped)
    for mask in range(total):
        signs = np.array([1.0 if mask & (1 << index) else -1.0 for index in range(len(grouped))])
        statistic = abs(float(np.average(grouped["mean"].to_numpy() * signs, weights=grouped["size"])))
        exceed += statistic >= observed - 1e-15
    return exceed / total


def spearman_row(frame: pd.DataFrame, predictor: str, outcome: str) -> tuple[int, float, float]:
    usable = frame[[predictor, outcome]].apply(pd.to_numeric, errors="coerce").dropna()
    if len(usable) < 3 or usable[predictor].nunique() < 2 or usable[outcome].nunique() < 2:
        return len(usable), np.nan, np.nan
    result = stats.spearmanr(usable[predictor], usable[outcome])
    return len(usable), float(result.statistic), float(result.pvalue)


def objective_findings(data: Mapping[str, pd.DataFrame], convergence: pd.DataFrame, proposal_task: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    current = convergence.copy()
    current["delta_shannon_entropy"] = numeric(current["delta_shannon_entropy"])
    overall_ci = building_bootstrap_interval(current, lambda sample: sample["delta_shannon_entropy"].mean(), seed=20261770)
    counts = current["exact_direction"].value_counts()
    findings: list[dict[str, Any]] = [{
        "finding_id": "paired_entropy_overall", "evidence_class": "current_outcome",
        "predictor_or_contrast": "Semi - Manual Shannon entropy", "outcome": "delta_shannon_entropy",
        "n_tasks": len(current), "n_buildings": current["building_id"].nunique(),
        "estimate": float(current["delta_shannon_entropy"].mean()), "ci_lower": overall_ci[0], "ci_upper": overall_ci[1],
        "p_value": exact_building_sign_flip(current, "delta_shannon_entropy"),
        "secondary_counts": json.dumps({"convergence": int(counts.get("semi_convergence", 0)), "no_change": int(counts.get("no_entropy_change", 0)), "expansion": int(counts.get("semi_expansion", 0))}, ensure_ascii=False),
        "interpretation_zh": "总体均值接近零且 building 区间跨零；当前没有净收敛或净扩散方向。",
    }]

    proxy = data["difficulty_proxy"].drop(columns=["delta_shannon_entropy_q095"], errors="ignore").copy()
    proxy = proxy.merge(current[["base_task_id", "building_id", "manual_shannon_entropy", "delta_shannon_entropy", "exact_direction"]], how="inner", on="base_task_id", validate="one_to_one")
    proxy["gt_pair_count"] = numeric(proxy["gt_pair_count"])
    proxy["legacy_occlusion"] = proxy["legacy_difficulty_label"].fillna("").str.split(";").map(lambda values: "occlusion" in values)
    for tag in ("occlusion", "seam", "trivial", "reflection", "low_texture"):
        proxy[f"manual_worker_tag__{tag}"] = proxy["manual_difficulty_choices_json"].map(lambda value, target=tag: target in (parse_jsonish(value) or []))

    n_ready = int(numeric(proxy["frozen_preassignment_n_ready"]).max()) if len(proxy) else 0
    findings.append({
        "finding_id": "formal_preassignment_risk", "evidence_class": "frozen_preassignment",
        "predictor_or_contrast": "frozen pre-assignment risk", "outcome": "delta_shannon_entropy",
        "n_tasks": len(proxy), "n_buildings": proxy["building_id"].nunique(), "estimate": np.nan,
        "ci_lower": np.nan, "ci_upper": np.nan, "p_value": np.nan,
        "secondary_counts": json.dumps({"n_ready": n_ready}, ensure_ascii=False),
        "interpretation_zh": "正式冻结的标注前 predictor 当前不可用；结论是不可评价，不是没有关系。",
    })
    n, rho, p_value = spearman_row(proxy, "gt_pair_count", "delta_shannon_entropy")
    rho_ci = building_bootstrap_interval(proxy, lambda sample: spearman_row(sample, "gt_pair_count", "delta_shannon_entropy")[1], seed=20261771)
    findings.append({
        "finding_id": "legacy_gt_pair_count", "evidence_class": "legacy_preoutcome_proxy",
        "predictor_or_contrast": "gt_pair_count", "outcome": "delta_shannon_entropy",
        "n_tasks": n, "n_buildings": proxy["building_id"].nunique(), "estimate": rho,
        "ci_lower": rho_ci[0], "ci_upper": rho_ci[1], "p_value": p_value, "secondary_counts": "",
        "interpretation_zh": "旧代理与当前结果的探索性相关；未检出稳定关系。",
    })

    def group_difference(sample: pd.DataFrame, field: str) -> float:
        yes = sample.loc[sample[field], "delta_shannon_entropy"]
        no = sample.loc[~sample[field], "delta_shannon_entropy"]
        return float(yes.mean() - no.mean()) if len(yes) and len(no) else np.nan

    grouped_fields = [("legacy_occlusion", "legacy_preoutcome_proxy")] + [
        (f"manual_worker_tag__{tag}", "worker_postresponse_tag") for tag in ("occlusion", "seam", "trivial", "reflection", "low_texture")
    ]
    for field, evidence_class in grouped_fields:
        yes = proxy[field]
        yes_expansion = int((yes & proxy["exact_direction"].eq("semi_expansion")).sum())
        no_expansion = int((~yes & proxy["exact_direction"].eq("semi_expansion")).sum())
        table = [[yes_expansion, int(yes.sum()) - yes_expansion], [no_expansion, int((~yes).sum()) - no_expansion]]
        fisher_p = float(stats.fisher_exact(table).pvalue) if min(int(yes.sum()), int((~yes).sum())) else np.nan
        interval = building_bootstrap_interval(proxy, lambda sample, name=field: group_difference(sample, name), seed=SEED + len(findings))
        findings.append({
            "finding_id": field, "evidence_class": evidence_class,
            "predictor_or_contrast": f"{field}: present - absent", "outcome": "delta_shannon_entropy",
            "n_tasks": len(proxy), "n_buildings": proxy["building_id"].nunique(), "estimate": group_difference(proxy, field),
            "ci_lower": interval[0], "ci_upper": interval[1], "p_value": fisher_p,
            "secondary_counts": json.dumps({"present_n": int(yes.sum()), "absent_n": int((~yes).sum()), "present_expansion": yes_expansion, "absent_expansion": no_expansion}, ensure_ascii=False),
            "interpretation_zh": "旧代理仅作候选线索。" if field == "legacy_occlusion" else "工人响应后标签不是独立标注前图像特质。",
        })

    n, rho, p_value = spearman_row(current, "manual_shannon_entropy", "delta_shannon_entropy")
    findings.append({
        "finding_id": "manual_entropy_algebraic_coupling", "evidence_class": "algebraically_coupled_baseline",
        "predictor_or_contrast": "manual_shannon_entropy", "outcome": "delta_shannon_entropy",
        "n_tasks": n, "n_buildings": current["building_id"].nunique(), "estimate": rho,
        "ci_lower": np.nan, "ci_upper": np.nan, "p_value": p_value, "secondary_counts": "",
        "interpretation_zh": "ΔH 含有 -H_Manual，存在代数耦合、地板效应和回归均值；不能解释为独立难度效应。",
    })

    proposal_rows: list[dict[str, Any]] = []
    for predictor in ("edit_rate", "edit_rmse_mean", "negative_metric_change_count", "delta_quality_iou"):
        n, rho, p_value = spearman_row(proposal_task, predictor, "delta_shannon_entropy")
        interval = building_bootstrap_interval(proposal_task, lambda sample, name=predictor: spearman_row(sample, name, "delta_shannon_entropy")[1], seed=SEED + 100 + len(proposal_rows))
        proposal_rows.append({
            "predictor": predictor, "outcome": "delta_shannon_entropy", "n_tasks": n,
            "n_buildings": proposal_task["building_id"].nunique(), "spearman_rho": rho,
            "cluster_ci_lower": interval[0], "cluster_ci_upper": interval[1], "unadjusted_p": p_value,
            "evidence_class": "posttreatment_mechanism" if predictor != "delta_quality_iou" else "concurrent_outcome",
            "interpretation_zh": "共现/机制描述，不能作为首次分配前 predictor，也不能解释为编辑导致扩散。",
        })

    future_fields = pd.DataFrame([
        ("identity", "base_task_id; building_id; dataset_id; split", "标注前", "连接任务、building 与数据来源"),
        ("frozen_image_traits", "occlusion; seam; reflection; low_texture; open_boundary; scope_risk", "标注前独立评审并冻结", "形成不依赖工人结果的图像特质"),
        ("proposal_diagnostics", "proposal_topology; initial_validity; initial_IoU_if allowed; forced_fit_flags", "工人看到 proposal 前冻结", "区分 proposal 结构、强制拟合与后续响应"),
        ("assignment", "condition; assignment_probability; pool_version; seed; worker_image_exposure", "派发前冻结", "支持图像级/worker 级设计审计"),
        ("responses", "final_geometry; meta_tags; active_time; lead_time; edit_trace", "标注后", "作为 outcome 或 mechanism，不反标为标注前 predictor"),
        ("versions", "script_version; event_schema; reference_sha; proposal_sha; code_commit", "采集时写入", "保证时间、GT 与 proposal 口径可追溯"),
    ], columns=["field_group", "recommended_fields", "freeze_timing", "purpose_zh"])
    return proxy, pd.DataFrame(findings), pd.DataFrame(proposal_rows), future_fields
...
def image_file_index() -> dict[str, list[Path]]:
    ...
    skip_parts = {".git", ".worktrees", "node_modules", "venv", ".venv", "full_uncertainty_data_mining_20260821_v2", "full_uncertainty_data_mining_20260821_v3", "full_uncertainty_data_mining_20260821_v4"}
...
def resolve_images(tasks: Iterable[str], image_map: Mapping[str, str]) -> pd.DataFrame:
    ...
    for task in sorted(set(filter(None, map(clean, tasks)))):
        ...
        relative_candidates = [(candidate, candidate.relative_to(ROOT).as_posix()) for candidate in candidates]
        tracked_candidates = [(candidate, relative) for candidate, relative in relative_candidates if relative in tracked]
        chosen, relative = (tracked_candidates or relative_candidates or [(None, "")])[0]
        rows.append({
            "base_task_id": task, "building_id": building_from_task(task),
            "image_reference": reference, "remote_reference_available": bool(reference),
            "workspace_local_found": chosen is not None, "workspace_local_path": relative,
            "git_tracked_image_found": bool(tracked_candidates),
...
def build_evidence_markdown_report(out: Path, frames: Mapping[str, pd.DataFrame], validation: Mapping[str, Any]) -> str:
    convergence = frames["semi_convergence"]
    ...
    lines = [
        "# 360° 全景布局标注：复核证据附录（v4）",
        "",
        "## 0. 新增客观发现（当前 outcome 复核）",
        "",
        dataframe_markdown(frames["objective_findings"]),
        "",
        "Proposal 相关均为 post-treatment/concurrent 描述，不作标注前预测或因果解释：",
        "",
        dataframe_markdown(frames["proposal_correlations"]),
        "",
        "旧 22-task proxy 已删除旧 outcome 列并按 base_task_id 重新连接当前 v4 ΔH。正式冻结的 pre-assignment predictor n_ready=0，因此不可评价。±0.05 与 ±0.10 的方向敏感性保存在 `SEMI_25_CONVERGENCE_EXPANSION_ALL_IMAGES.csv`。",
        "",
        "## 1. 数据保留与分析边界",
        "",
        ...
        "",
        "每类分析的全部实例都通过 `base_task_id` 和 `image_reference` 列出。v4 只记录本机可定位与 Git 跟踪状态，不复制原图或生成缩略图分页。",
        "",
        "## 13. 统计和解释限制",
        "",
        ...
        "",
        "变量中文解释和近似计算见 `DATA_DICTIONARY_ZH_V4.csv`；输入/输出 SHA-256 见 `INPUT_PROVENANCE_V4.csv` 和 `OUTPUT_MANIFEST.csv`。",
    ]
    report = "\n".join(lines) + "\n"
...
def build_evidence_docx(out: Path, frames: Mapping[str, pd.DataFrame], charts: Sequence[Path]) -> Path:
    target = out / "Paper_A_复核证据附录_20260821_v4.docx"
    document = Document()
    ...
    styles["Normal"]._element.get_or_add_rPr().get_or_add_rFonts().set(qn("w:eastAsia"), "Microsoft YaHei")
    ...
    document.add_heading("新增客观发现（已复核）", level=1)
    add_docx_table(document, frames["objective_findings"], ["finding_id", "evidence_class", "n_tasks", "n_buildings", "estimate", "ci_lower", "ci_upper", "p_value", "secondary_counts", "interpretation_zh"], "当前 outcome 复核结果", None)
    add_docx_table(document, frames["proposal_correlations"], list(frames["proposal_correlations"].columns), "Proposal post-treatment / concurrent 相关", None)
    document.add_paragraph("旧 22-task proxy 已重连当前 v4 ΔH；正式冻结的 pre-assignment predictor n_ready=0，只能报告不可评价。")
    ...
    add_docx_table(document, frames["proposal_task"], ["base_task_id", "delta_shannon_entropy", "edit_rate", "edit_rmse_median", "positive_metric_change_count", "negative_metric_change_count", "proposal_response_pattern", "anchor_evidence", "forced_fit_tradeoff_evidence"], "Proposal 任务级审计", 25)
    add_docx_table(document, frames["tag_behavior_cases"], ["stage", "base_task_id", "worker_id", "tag_codes", "case_codes", "edited_bool", "edit_magnitude_px_equivalent", "topology_changed", "iou_to_gt", "cluster_rank", "metric_change_class"], "标签—行为实例（前 30 行；全量见 CSV/XLSX）", 30)
    ...
    add_docx_table(document, frames["crowd_gt_causes"], ["base_task_id", "condition", "crowd_gt_relationship", "largest_cluster_support", "second_cluster_support", "best_cluster_rank", "crowd_gt_gap_best_minus_largest", "observable_geometric_difference_codes", "manual_review_priority"], "Crowd–GT 实例（前 25 行；全量 101 行见 CSV/XLSX）", 25)
    ...
    add_docx_table(document, frames["dual_annotator"], ["stage", "condition", "base_task_id", "worker_id_left", "worker_id_right", "topology_agreement_class", "n_corners_left", "n_corners_right", "cyclic_rmse_diagonal_normalized", "coordinate_disagreement_band"], "双人任务（前 25 行；全量 54 行见 CSV/XLSX）", 25)
    ...
    add_docx_table(document, frames["image_index"], list(frames["image_index"].columns), "图片实例索引（前 20 行；全量 214 行见 CSV/XLSX）", 20)

    document.add_page_break()
    document.add_heading("图表", level=1)
    for path in charts[:2]:
        if path.is_file():
    ...
    dictionary_keys = {
        "negative_metric_change", "positive_metric_change", "near_zero_metric_change", "semi_convergence", "semi_expansion",
        "delta_shannon_entropy", "acceptable_proposal_with_any_corner_edit", "material_edit", "trivial_tag_but_low_GT_alignment",
        "orthogonal_flag_count", "cyclic_rmse_diagonal_normalized", "observable_geometric_difference_codes",
        "crowd_gt_gap_best_minus_largest", "coordinate_disagreement_band", "anchor_evidence", "structure_discovery_evidence",
        "forced_fit_tradeoff_evidence", "U_initial", "U_final", "delta_U", "formal_assignment_status", "semi_pairing_status",
        "lead_time_is_active_time",
    }
    dictionary = frames["dictionary"]
    dictionary_display = pd.concat([dictionary[dictionary["variable_en"].isin(dictionary_keys)], dictionary[~dictionary["variable_en"].isin(dictionary_keys)].head(17)])
    add_docx_table(document, dictionary_display, ["variable_en", "variable_zh", "meaning_zh", "approximate_formula"], "关键中英文数据词典（完整 55 项见 CSV/XLSX）", None)
...
def finding_row(findings: pd.DataFrame, finding_id: str) -> pd.Series:
    rows = findings[findings["finding_id"].eq(finding_id)]
    if len(rows) != 1:
        raise AssertionError(f"expected one objective finding {finding_id}, observed {len(rows)}")
    return rows.iloc[0]


def build_advisor_markdown(frames: Mapping[str, pd.DataFrame], validation: Mapping[str, Any]) -> str:
    findings = frames["objective_findings"]
    overall = finding_row(findings, "paired_entropy_overall")
    legacy = finding_row(findings, "legacy_occlusion")
    baseline = finding_row(findings, "manual_entropy_algebraic_coupling")
    proposal = frames["proposal_correlations"]
    exact_counts = frames["semi_convergence"]["exact_direction"].value_counts()
    tol05 = frames["semi_convergence"]["uncertainty_direction"].value_counts()
    lines = [
        "# Paper A：导师版客观发现与不确定性分析（v4）", "",
        "## 结论先行", "",
        f"当前 25 个 Manual/Semi 同图任务覆盖 {int(overall['n_buildings'])} 个 building。精确方向为：收敛 {int(exact_counts.get('semi_convergence', 0))}、完全不变 {int(exact_counts.get('no_entropy_change', 0))}、扩散 {int(exact_counts.get('semi_expansion', 0))}。平均 ΔH={overall['estimate']:.6f}，building bootstrap 95% CI [{overall['ci_lower']:.6f}, {overall['ci_upper']:.6f}]，精确 building sign-flip p={overall['p_value']:.6f}。证据支持“总体接近零、图像间异质”，不支持统一的净收敛或净扩散。", "",
        f"采用预先固定的 ±0.05 近零带时，收敛/近零/扩散为 {int(tol05.get('semi_convergence', 0))}/{int(tol05.get('near_zero_change', 0))}/{int(tol05.get('semi_expansion', 0))}。因此，论文最稳妥的研究对象不是‘Semi 是否普遍更好’，而是 proposal 如何改变标注分布、在哪类图像上收敛或扩散，以及标签—操作—几何之间何时不一致。", "",
        "## 1. 已复核的新增客观发现", "",
        dataframe_markdown(findings, ["finding_id", "evidence_class", "n_tasks", "estimate", "ci_lower", "ci_upper", "p_value", "secondary_counts", "interpretation_zh"]), "",
        "正式冻结的标注前风险变量目前 n_ready=0，因此只能写‘不可评价’，不能写‘没有关系’。旧版 22-task proxy 已重新连接到当前 v4 ΔH，未继承旧文件中的过时 outcome 列。", "",
        f"旧 occlusion proxy 的 present−absent 平均 ΔH 差为 {legacy['estimate']:.6f}，但样本仅 22 个任务、区间宽且 Fisher p={legacy['p_value']:.6f}，只能作为后续预注册候选。Manual entropy 与 ΔH 的 Spearman ρ={baseline['estimate']:.6f}（p={baseline['p_value']:.6g}）主要受 ΔH=H_Semi−H_Manual 的代数耦合影响，不能解释为独立难度效应。", "",
        "## 2. Proposal 编辑行为", "",
        dataframe_markdown(proposal), "",
        ...
        "## 4. 数据语义与可追溯性", "",
        f"raw 记录 {validation['raw_record_count']} 条，selected/canonical 记录 {validation['selected_record_count']} 条，raw-only 可审计版本 {validation['raw_only_record_count']} 条。raw-only 只按 stage/project/task/worker 上下文映射到 selected 记录，不推断版本时间先后。", "",
        dataframe_markdown(frames["time_summary"]), "",
...
def set_docx_business_style(document: Document, title_text: str, subtitle: str) -> None:
    section = document.sections[0]
    ...
    for style_name, size, color in (("Title", 23, "17365D"), ("Heading 1", 15, "17365D"), ("Heading 2", 11, "2F5597")):
        style = document.styles[style_name]
        style.font.name, style.font.size, style.font.color.rgb = "Arial", Pt(size), RGBColor.from_string(color)
        style._element.get_or_add_rPr().get_or_add_rFonts().set(qn("w:eastAsia"), "Microsoft YaHei")
    header = section.header.paragraphs[0]
    header.text = "PAPER A  /  OBJECTIVE EVIDENCE BRIEF  /  2026-08-21"
    header.runs[0].font.size = Pt(7.5)
    footer = section.footer.paragraphs[0]
    footer.alignment = WD_ALIGN_PARAGRAPH.CENTER
    footer.add_run("HOHONET · v4 · ")
    ...
    document.add_paragraph("结论版本：客观复核后写入；探索性发现均保留证据等级与不可作出的因果解释。")


def build_advisor_docx(out: Path, frames: Mapping[str, pd.DataFrame], charts: Sequence[Path]) -> Path:
    target = out / "Paper_A_导师版客观发现与不确定性分析_20260821_v4.docx"
    findings = frames["objective_findings"]
    overall = finding_row(findings, "paired_entropy_overall")
    document = Document()
    set_docx_business_style(document, "360° 全景布局标注", "导师版客观发现与不确定性分析 · v4")
    document.add_heading("结论先行", level=1)
    document.add_paragraph(
        f"25 个配对任务、{int(overall['n_buildings'])} 个 building：10 收敛、4 完全不变、11 扩散。"
        f"平均 ΔH={overall['estimate']:.4f}，building bootstrap 95% CI [{overall['ci_lower']:.4f}, {overall['ci_upper']:.4f}]，"
        f"精确 sign-flip p={overall['p_value']:.4f}。总体接近零，但任务级异质性明显。"
    )
    document.add_paragraph("主线建议：把问题表述为 proposal 对分布、残余调整和多模态结构的影响；避免把指标下降直接叫作错误修改，也避免把共识直接等同于真值。")
    add_docx_table(document, frames["semi_convergence"], ["base_task_id", "delta_shannon_entropy", "exact_direction"], "25 个配对任务", 25)
    document.add_page_break()
    document.add_heading("新增客观发现（已复核）", level=1)
    key_findings = findings[findings["finding_id"].isin(["paired_entropy_overall", "formal_preassignment_risk", "legacy_gt_pair_count", "legacy_occlusion", "manual_entropy_algebraic_coupling"])]
    add_docx_table(document, key_findings, ["finding_id", "evidence_class", "n_tasks", "estimate", "ci_lower", "ci_upper", "p_value"], "关键任务级发现（完整 10 项见附录）", None)
    document.add_paragraph("正式冻结的标注前风险变量 n_ready=0：不可评价，不等于没有关系。Manual entropy 与 ΔH 存在代数耦合，不能作为独立难度效应。")
    document.add_page_break()
    document.add_heading("Proposal 与标签—操作关系", level=1)
    add_docx_table(document, frames["proposal_correlations"], list(frames["proposal_correlations"].columns), "Proposal 机制相关", None)
    add_docx_table(document, frames["tag_behavior_summary"], ["stage", "case_code", "row_count", "task_count", "worker_count", "median_iou", "median_edit_px", "negative_metric_change_rate"], "标签—行为案例汇总（前 8 类；完整见附录）", 8)
    document.add_page_break()
    document.add_heading("Crowd、少数模式与 GT", level=1)
    add_docx_table(document, frames["cause_summary"], list(frames["cause_summary"].columns), "101 个 task-condition 的可观察差异汇总", None)
    add_docx_table(document, frames["dual_summary"], list(frames["dual_summary"].columns), "54 个双人任务敏感性", None)
    document.add_paragraph("双人任务只能证明成对分歧是否明显，不能形成多数共识或替代 GT。")
    document.add_page_break()
    document.add_heading("数据边界与可追溯性", level=1)
    add_docx_table(document, frames["time_summary"], ["time_measurement_lane", "record_count", "active_time_observed_count", "lead_time_observed_count"], "Active time / Lead time 分层", None)
    document.add_paragraph("逐记录 script_version 与 not_recorded 状态见 TIME_MEASUREMENT_RECORD_AUDIT.csv；Lead time 不进入 Active time 汇总。")
    add_docx_table(document, frames["image_summary"], list(frames["image_summary"].columns), "图片可用性", None)
    add_docx_table(document, frames["future_fields"], list(frames["future_fields"].columns), "下一轮冻结字段", None)
    document.add_paragraph("v4 不打包 ZIP、原图或缩略图；完整逐行数据在同目录 CSV 和工作簿。")
    for path in charts[:2]:
        if path.is_file():
            document.add_page_break()
            document.add_heading(path.stem, level=1)
            document.add_picture(str(path), width=Inches(6.5))
    document.save(target)
    if len(Document(target).tables) < 7:
        raise AssertionError("advisor DOCX validation failed")
    return target
...
def build_xlsx(out: Path, frames: Mapping[str, pd.DataFrame], charts: Sequence[Path]) -> Path:
    target = out / "Paper_A_完整数据整理与分析工作簿_20260821_v4.xlsx"
    workbook = Workbook()
    workbook.remove(workbook.active)
    sheet_map = [
        ("README_说明", frames["readme"]),
        ("Coverage_覆盖", frames["coverage"]),
        ("Findings_客观发现", frames["objective_findings"]),
        ("ProposalCorr_相关", frames["proposal_correlations"]),
        ("Proxy22_旧代理", frames["difficulty_proxy_current"]),
        ...
        ("Raw2513_总账", frames["raw_ledger"]),
        ("Raw12_映射", frames["raw_crosswalk"]),
        ("Time_逐行", frames["time_records"]),
        ("Time_汇总", frames["time_summary"]),
        ("Worker_综合", frames["worker_integrated"]),
        ("Image_图片索引", frames["image_index"]),
        ("Image_汇总", frames["image_summary"]),
        ("Future_字段", frames["future_fields"]),
...
def data_dictionary_v4() -> pd.DataFrame:
    rows = [
        ...
        ("U_initial", "初始效用", "proposal 相对冻结参考的 IoU；P1 采用普通几何，C1 采用其冻结规范化口径。初始几何无效且参考有效时记 0；参考不可用时缺失。", "IoU(proposal, frozen reference)"),
        ("U_final", "最终效用", "最终几何相对同一冻结参考的效用；C1 使用冻结 iou_to_gt。工人造成的无效几何且参考有效时记 0；其他不可计算情形缺失。", "IoU(final, same frozen reference)"),
        ("delta_U", "效用变化", "最终效用减初始效用；主描述阈值为 ±0.01，并另保留 0、0.02、0.05 敏感性网格。", "U_final - U_initial"),
        ("formal_assignment_status", "正式分配资格状态", "仅在适用阶段使用权威 formal_assignment_eligible；P1 标为 not_applicable，缺失标为 not_recorded。", "eligible|ineligible|not_recorded|not_applicable"),
        ("semi_pairing_status", "Manual/Semi 配对状态", "C1 的 Semi、同图配对 Manual 与 Manual-only 分开编码；非 C1 不适用。", "design-aware categorical status"),
        ("lead_time_is_active_time", "Lead time 是否等于 Active time", "恒为 False；Label Studio 经过时间与页面有效标注时长分开分析。", "False"),
    ]
...
def input_provenance() -> pd.DataFrame:
    ...
    rows = []
    for path in paths:
        is_ephemeral = V2 in path.parents
        rows.append({
            "path": f"ephemeral_reviewed_derivative/{path.name}" if is_ephemeral else path.relative_to(ROOT).as_posix(),
            "exists_at_generation": path.is_file(),
            "source_role": "ephemeral_reviewed_derivative" if is_ephemeral else "repository_source_or_contract",
...
def materialize(out: Path, derived_v2: Path | None = None) -> dict[str, Any]:
    global V2
    if out.exists():
        shutil.rmtree(out)
    out.mkdir(parents=True)
    generated_derivative = derived_v2 is None
    ...
    if not (V2 / "UNIFIED_SUBMISSION_EVIDENCE_REVIEWED.csv").is_file():
        raise FileNotFoundError(f"reviewed derivative incomplete: {V2}")
    data = load_data()
    image_map = build_image_reference_map(data["raw"])
    flagged, exclusion_summary, exclusion_outcomes, exclusion_overlap = orthogonal_record_flags(data["unified"], data["row_inclusion"])
    time_records, time_summary = time_measurement_audit(flagged, data["raw_active_events"])
    flagged = time_records
    out_rows, out_summary = raw_out_of_task(data["raw"], flagged)
    raw_ledger, raw_crosswalk = raw_record_ledger(data["raw"])
    ...
    difficulty_proxy_current, findings, proposal_correlations, future_fields = objective_findings(data, convergence, proposal_task)
    ...
    image_summary = pd.DataFrame([{
        "task_count": len(image_index),
        "remote_reference_count": int(image_index["remote_reference_available"].sum()),
        "workspace_local_found_count": int(image_index["workspace_local_found"].sum()),
        "git_tracked_image_count": int(image_index["git_tracked_image_found"].sum()),
        "delivery_contains_images": False,
        "interpretation_zh": "本机存在不等于 Git 仓库证据；v4 不复制原图或生成 gallery。",
    }])
    dictionary = data_dictionary_v4()
    ...
    readme = pd.DataFrame([
        ...
        {"section": "Image instances", "description_zh": "所有实例保留远程引用，并分开标记 workspace-local 与 Git-tracked；不复制原图。"},
        {"section": "Objective findings", "description_zh": "新增客观分析经当前 v4 outcome 重连与独立复算后写入；探索性、post-treatment 与代数耦合证据分级。"},
    ])
    frames: dict[str, pd.DataFrame] = {
        "readme": readme,
        "coverage": coverage,
        "objective_findings": findings,
        "proposal_correlations": proposal_correlations,
        "difficulty_proxy_current": difficulty_proxy_current,
    ...
    csv_outputs = {
        "OBJECTIVE_FINDINGS_REVIEWED.csv": findings,
        "PROPOSAL_POSTTREATMENT_CORRELATIONS.csv": proposal_correlations,
        "DIFFICULTY_PROXY_22_REJOINED_TO_CURRENT_OUTCOME.csv": difficulty_proxy_current,
        "FUTURE_FROZEN_FIELD_PLAN.csv": future_fields,
        ...
        "RAW_ANNOTATION_LEDGER_ALL_2513.csv": raw_ledger,
        "RAW_ONLY_TO_SELECTED_CONTEXT_CROSSWALK.csv": raw_crosswalk,
        "TIME_MEASUREMENT_RECORD_AUDIT.csv": time_records,
        "TIME_MEASUREMENT_SOURCE_SUMMARY.csv": time_summary,
        ...
        "IMAGE_AVAILABILITY_SUMMARY.csv": image_summary,
        "DATA_DICTIONARY_ZH_V4.csv": dictionary,
        "INPUT_PROVENANCE_V4.csv": provenance,
    }
    for name, frame in csv_outputs.items():
        write_csv(out / name, frame)
    charts = chart_outputs(out, frames)
    exact_counts = convergence["exact_direction"].value_counts()
    tol05_counts = convergence["uncertainty_direction"].value_counts()
    tol10_counts = convergence["direction_tolerance_010"].value_counts()
    c1 = flagged[flagged["stage"].eq("C1")]
    validation = {
        "paired_manual_semi_task_count": len(convergence),
        "paired_building_count": convergence["building_id"].nunique(),
        "exact_convergence_count": int(exact_counts.get("semi_convergence", 0)),
        "exact_no_change_count": int(exact_counts.get("no_entropy_change", 0)),
        "exact_expansion_count": int(exact_counts.get("semi_expansion", 0)),
        "tolerance_005_counts": tol05_counts.to_dict(),
        "tolerance_010_counts": tol10_counts.to_dict(),
        ...
        "raw_record_count": len(raw_ledger),
        "selected_record_count": int(raw_ledger["canonical_join_status"].eq("matched").sum()),
        "raw_only_record_count": len(raw_crosswalk),
        "c1_formal_assignment_eligible_count": int(c1["formal_assignment_status"].eq("eligible").sum()),
        "c1_formal_assignment_ineligible_count": int(c1["formal_assignment_status"].eq("ineligible").sum()),
        "c1_manual_only_record_count": int(c1["semi_pairing_status"].eq("manual_only_no_semi_candidate").sum()),
        "formal_preassignment_n_ready": int(numeric(difficulty_proxy_current["frozen_preassignment_n_ready"]).max()),
        "delivery_contains_zip": False,
        "delivery_contains_gallery_or_original_images": False,
        "negative_metric_change_terminology_only": True,
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
    }
    expected = {
        "paired_manual_semi_task_count": 25, "paired_building_count": 9,
        "exact_convergence_count": 10, "exact_no_change_count": 4, "exact_expansion_count": 11,
        "crowd_gt_task_condition_count": 101, "dual_annotator_task_count": 54,
        "raw_record_count": 2513, "selected_record_count": 2501, "raw_only_record_count": 12,
        "c1_formal_assignment_eligible_count": 771, "c1_formal_assignment_ineligible_count": 9,
        "c1_manual_only_record_count": 539, "formal_preassignment_n_ready": 0,
    }
    mismatches = {key: (validation.get(key), value) for key, value in expected.items() if validation.get(key) != value}
    if mismatches or convergence["base_task_id"].duplicated().any() or c1.loc[c1["condition"].eq("semi"), "semi_pairing_status"].eq("manual_only_no_semi_candidate").any():
        raise AssertionError({"mismatches": mismatches, "duplicate_pair_task": convergence["base_task_id"].duplicated().any()})
    if tol05_counts.to_dict() != {"semi_expansion": 11, "semi_convergence": 10, "near_zero_change": 4}:
        raise AssertionError({"tolerance_005_counts": tol05_counts.to_dict()})
    if tol10_counts.to_dict() != {"semi_expansion": 10, "semi_convergence": 9, "near_zero_change": 6}:
        raise AssertionError({"tolerance_010_counts": tol10_counts.to_dict()})
    if time_records["lead_time_is_active_time"].any():
        raise AssertionError("lead_time entered active-time lane")
    advisor_md = out / "PAPER_A_ADVISOR_OBJECTIVE_FINDINGS_ZH_V4.md"
    advisor_md.write_text(build_advisor_markdown(frames, validation), encoding="utf-8", newline="\n")
    evidence_md = out / "PAPER_A_REVIEWED_EVIDENCE_APPENDIX_ZH_V4.md"
    evidence_md.write_text(build_evidence_markdown_report(out, frames, validation), encoding="utf-8", newline="\n")
    advisor_docx = build_advisor_docx(out, frames, charts)
    evidence_docx = build_evidence_docx(out, frames, charts)
    xlsx = build_xlsx(out, frames, charts)
    (out / "SUPERSESSION_NOTICE.md").write_text(
        "# v4 supersession notice\n\nv4 replaces v3. It corrects assignment/pairing semantics, rebuilds the reviewed derivative, "
        "rejoins legacy proxies to the current outcome, and adds independently checked objective findings.\n",
        encoding="utf-8", newline="\n",
    )
    (out / "REMOTE_DELIVERY_INDEX.md").write_text(
        "# v4 delivery index\n\n"
        f"- `{advisor_md.name}` / `{advisor_docx.name}` — advisor brief\n"
        f"- `{evidence_md.name}` / `{evidence_docx.name}` — reviewed evidence appendix\n"
        f"- `{xlsx.name}` — complete workbook\n"
        "- canonical CSV tables and SHA manifest are stored beside these files.\n"
        "- no ZIP, original images, galleries or copied analysis-code directory.\n",
        encoding="utf-8", newline="\n",
    )
    if generated_derivative:
        shutil.rmtree(V2)
    validation.update({
        "advisor_docx_size_bytes": advisor_docx.stat().st_size,
        "evidence_docx_size_bytes": evidence_docx.stat().st_size,
        "xlsx_size_bytes": xlsx.stat().st_size,
        "output_file_count": len([path for path in out.rglob("*") if path.is_file()]) + 2,
    })
...
def main() -> None:
    ...
    parser.add_argument("--derived-v2-dir", type=Path, help="reuse an already generated reviewed v2 directory")
    args = parser.parse_args()
    materialize(args.output_dir.resolve(), args.derived_v2_dir.resolve() if args.derived_v2_dir else None)
