...
def run(out_dir: Path = OUT_DIR, review_out_dir: Path = REVIEW_OUT_DIR):
    ...
    cores = [("s4", value, {4: {"x": value, "bottom_y": 0}, 9: {"x": -0.05, "bottom_y": 0}, 10: {"x": 0.05, "bottom_y": 0}}) for value in (-0.15, -0.30, -0.40)]
    cores += [("s9", value, {4: {"x": -0.05, "bottom_y": 0}, 9: {"x": value, "bottom_y": 0}, 10: {"x": 0.05, "bottom_y": 0}}) for value in (-0.15, -0.30, -0.50, -1.00)]
    visibility = []
    for index, (pair, value, core) in enumerate(cores, start=1):
        ...
        row.update(visibility_slice=True, review_bucket="visibility_only", sensitivity_only=True, final_refinement_eligible=False, m_anchor_4_2_input_eligible=False)
        visibility.append(row)
    for row in stage_d:
        row.update(visibility_slice=False, review_bucket="stage_d_sensitivity_only", sensitivity_only=True, final_refinement_eligible=False, m_anchor_4_2_input_eligible=False)
    ...
    manifest["visibility_candidates"] = [{"candidate_id": row["candidate_id"], "family": "m_anchor_4_1_2_1_directional_visibility", "decision_class": row["candidate_class"], "coordinate_changes": row["coordinate_changes"], "sensitivity_only": True, "final_refinement_eligible": False, "requires_explicit_human_visual_verdict": True, "direct_ls_trial_allowed": False, "m_anchor_4_2_input_eligible": False, "accepted": False, "annotation_writeback": False, "review_bucket": "visibility_only"} for row in visibility]
