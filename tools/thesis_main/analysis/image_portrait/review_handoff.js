...
(() => {
 ...
 const key=d.focus?'image-difficulty-key39-20260916-v1':'image-difficulty-review-20260915-v1',schema='image_difficulty_user_review_v1';
 ...
 function progress(){$r('status').textContent=`${rows.size}图／${units.size}个Manual或Semi单元；${d.focus?'44项重点问题，含9张OOS；':''}已审核 ${Object.values(edits).filter(v=>v.status==='已审核').length}，暂缓 ${Object.values(edits).filter(v=>v.status==='暂缓').length}。`;}
 ...
 function render(){const r=rows.get(dataset.cases[currentCase].image_id);if(!r)return;$r('image').value=r.image_id;$r('mode').replaceChildren(...r.units.map(u=>opt(u.condition,u.condition)));$r('note').value=notes[r.image_id]||'';$r('original').src=(d.image_root||'../../../')+r.image_path;$r('original-link').href=$r('original').src;mode();progress();if(d.focus)questions(r);}
 function choices(){const q=$r('search').value.toLowerCase();$r('image').replaceChildren(...d.rows.filter(r=>[r.image_id,r.number,r.scene,...(r.questions||[]).map(x=>x.question+' '+x.trigger)].join(' ').toLowerCase().includes(q)).map(r=>opt(r.image_id,`${r.priority?'优先 · ':''}${r.building} · ${r.number} · ${r.scene}`)));}
 ...
 if(d.focus){panel.querySelector('h2').textContent='39张关键图片 · 44项证据审核';panel.append(questionBox);}
 const triggers={medium_partition_weak_separation:'中等候选：数值簇是否真实分离',model_high_human_low:'模型分歧高／真人指标低',model_low_human_high:'模型分歧低／真人指标高',same_room_different_candidates:'同房视角候选不同',recovered_OOS_geometry:'OOS几何与范围解释'};
 function questions(r){
  questionBox.replaceChildren();
  for(const q of r.questions||[]){const card=document.createElement('article');card.className='history-card';
   card.innerHTML=`<h3>${E(q.question_id)} · ${E(triggers[q.trigger]||q.trigger)} · ${E(q.condition)}</h3><p><b>${E(q.question)}</b></p><p>${E(q.location)}</p><p>数值依据：${E(q.numerical_evidence)}；簇人数：${E(q.numeric_cluster_sizes)}</p><p>影响判断：${E(q.affected_conclusion)}</p><p>请在上方图片观察中按问题编号记录判断；OOS不自动套用普通难度类。</p>`;
   const buttons=document.createElement('div');const modeButton=document.createElement('button');modeButton.textContent='查看本问题条件的全部簇';modeButton.onclick=()=>{const s=document.getElementById('history-mode');s.value=q.condition==='oos_geometry'?'OOS':q.condition==='manual'?'Manual':'Semi';s.dispatchEvent(new Event('change'));document.getElementById('history-clusters').scrollIntoView({behavior:'smooth'});};buttons.append(modeButton);
   for(const cid of q.representative_canonical_ids.split(';').filter(Boolean)){const i=dataset.cases[currentCase].variants.findIndex(v=>v.source?.canonical_annotation_id===cid);if(i<0)throw Error('重点作答未绑定：'+cid);const v=dataset.cases[currentCase].variants[i],b=document.createElement('button');b.textContent=`代表作答 ${v.source.worker_id} · 3D`;b.onclick=()=>{document.getElementById('variant-select').value=i;chooseVariant(i,true);document.querySelector('.workspace').scrollIntoView({behavior:'smooth'});};buttons.append(b);}
   card.append(buttons);questionBox.append(card);
  }
  if(!r.units.length){$r('evidence').textContent='此图为OOS几何专项；查看下方问题和OOS分簇，不自动生成普通难度候选。';const s=document.getElementById('history-mode');s.value='OOS';s.dispatchEvent(new Event('change'));}
 }
 document.addEventListener('studio-case',render);$r('save').textContent=message;choices();if(dataset.cases[currentCase].history_loaded)render();
