...
def run(root=R, output_dir=None):
 ...
 cache=json.loads((output_dir/'cache.json').read_text(encoding='utf-8'));s=[];m=[];centres=[]
 ...
 pd.DataFrame(s).to_csv(output_dir/'exemplar_summary.csv',index=False);pd.DataFrame(m).to_csv(output_dir/'exemplar_memberships.csv',index=False);pd.DataFrame(centres).to_csv(output_dir/'exemplar_centres.csv',index=False)
 (output_dir/'cache_with_exemplars.json').write_text(json.dumps(cache,separators=(',',':')), encoding='utf-8')
 rv=pd.read_csv(output_dir/'user24_multitarget_relations.csv');results=[]
 ...
 rv.to_csv(output_dir/'exemplar_user24.csv',index=False)
