...
def run(root=R, output_dir=None):
 ...
 rows,audit=lp.load(R);by={r['canonical_annotation_id']:r for r in rows}
 original=json.loads((output_dir/'cache.json').read_text(encoding='utf-8'));cache=json.loads((output_dir/'cache.json').read_text(encoding='utf-8'))
 ...
 pd.DataFrame(modifications).to_csv(output_dir/'no_borrowed_changes.csv',index=False)
 pd.DataFrame(members).to_csv(output_dir/'no_borrowed_changed_memberships.csv',index=False)
 pd.DataFrame(out).to_csv(output_dir/'no_borrowed_next_person.csv',index=False)
 (output_dir/'cache_no_borrowed.json').write_text(json.dumps(cache,separators=(',',':')), encoding='utf-8')
