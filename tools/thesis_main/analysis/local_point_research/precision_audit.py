...
def run(root=R, output_dir=None):
 ...
 rows,au=lp.load(R);rr={r['canonical_annotation_id']:r for r in rows}
 p=pd.read_csv(output_dir/'pairwise.csv');metrics=[]
 ...
 d=pd.DataFrame(metrics);d.to_csv(output_dir/'precision_pairwise.csv',index=False)
 cache=json.loads((output_dir/'cache.json').read_text(encoding='utf-8'));lookup={tuple(sorted((x.id_a,x.id_b))):x for x in d.itertuples()};impact=[]
 ...
 pd.DataFrame(impact).to_csv(output_dir/'precision_partition_impact.csv',index=False)
