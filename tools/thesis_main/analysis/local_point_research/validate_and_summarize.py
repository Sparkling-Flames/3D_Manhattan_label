...
def run(root=R, output_dir=None):
 ...
 rows,audit=lp.load(R);valid=[r for r in rows if lp.point_ok(r)];rng=np.random.default_rng(20260919)
 ...
 cache=json.loads((output_dir/'cache.json').read_text(encoding='utf-8'));g=next(g for g in cache.values()if len(g['ids'])==5)
 ...
 pd.DataFrame(synthetic).to_csv(output_dir/'synthetic_stress.csv',index=False)
 p=pd.read_csv(output_dir/'pairwise.csv');strata=[]
 for rad in [6,9,12]:
 ...
 pd.DataFrame(strata).to_csv(output_dir/'local_strata.csv',index=False)
 ...
 (output_dir/'TESTS.json').write_text(json.dumps(tests,ensure_ascii=False,indent=2), encoding='utf-8')
