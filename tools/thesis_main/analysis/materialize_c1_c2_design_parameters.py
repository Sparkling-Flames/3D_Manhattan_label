...
def _fit_crossed_model(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Fit one declared crossed variance-component model; never fall back."""
    support = {
    ...
    try:
        ...
        values = [*components.values(), float(result.scale), fixed]
        finite = all(math.isfinite(value) and (value >= 0 or value == fixed) for value in values)
        status = "estimated" if bool(result.converged) and finite and not convergence_warnings else "not_converged_or_singular"
        return {
            "status": status,
            "formula": formula,
            "optimizer": "lbfgs",
            ...
            "between_worker_slope_sd": math.sqrt(max(0.0, components.get("worker_slope", math.nan))),
    ...
    *,
    ...
    for worker in sorted(completion):
        ...
        rows.append({
            ...
            "group_prior_slope": group_slope, "group_prior_scale": group_sd, "group_slope_mean": group_slope, "group_slope_sd": "",
    ...
    formal_ready = bool(eligible_rows) and model.get("status") == "estimated" and all(row["Q_GT_baseline_se"] != "" for row in eligible_rows)
    summary = {
        ...
        "variance_components": {name: model.get(name, "") for name in ("between_worker_slope_sd", "outcome_residual_sd", "worker_intercept_sd", "task_sd", "building_sd")},
    }
