...
def materialize_canonical_evidence(export_paths: list[Path], canonical_csv: Path, output_dir: Path, *, stage: str = "C1", input_status: str = "dry_run") -> dict[str, Any]:
    ...
    for export_path in export_paths:
        ...
        for task_index, task in enumerate(_load_tasks(export_path), 1):
            ...
            for ann_index, annotation in enumerate(task.get("annotations") or [], 1):
                ...
                meta.update({key: _safe_str(source.get(key)) for key in ("duplicate_worker_task_submission", "duplicate_review_status", "duplicate_decision", "independence_audit_source_sha256", "independence_audit_snapshot_path", "independence_audit_snapshot_sha256", "independence_audit_identity")})
    ...
    meta_fields += ["duplicate_worker_task_submission", "duplicate_review_status", "duplicate_decision", "independence_audit_source_sha256", "independence_audit_snapshot_path", "independence_audit_snapshot_sha256", "independence_audit_identity"]
