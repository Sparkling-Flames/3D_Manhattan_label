...
def materialize_canonical_evidence(export_paths: list[Path], canonical_csv: Path, output_dir: Path, *, stage: str = "C1", input_status: str = "dry_run", version_disposition_csv: Path | None = None) -> dict[str, Any]:
    ...
    for row, record in selected_records:
        ...
        meta.update({key: _safe_str(row.get(key)) for key in ("w034_active_time_validation_manifest_sha256", "w034_active_time_validation_status", "timing_not_evaluable_reason")})
    ...
    meta_fields[meta_fields.index("active_time_expected") + 1:meta_fields.index("active_time_expected") + 1] = ["w034_active_time_validation_manifest_sha256", "w034_active_time_validation_status", "timing_not_evaluable_reason"]
