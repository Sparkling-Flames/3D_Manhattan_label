...
def estimate_stage_stacked_components(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Fail closed until a C2-B-confirmed cross-stage estimator is frozen."""
    grouped: dict[str, list[dict[str, Any]]] = {}
    for row in rows: grouped.setdefault(str(row.get("component_family", "")), []).append(row)
    output = []
    for family, items in sorted(grouped.items()):
        workers = {str(row.get("worker_id", "")) for row in items if str(row.get("worker_id", ""))}
        tasks = {str(row.get("base_task_id", "")) for row in items if str(row.get("base_task_id", ""))}
        output.append({
            "component_family": family, "combined_effect": "", "combined_interval": "",
            "stage_interaction": "", "stage_reversal_flag": "not_evaluable",
            "worker_support": len(workers), "task_support": len(tasks),
            "component_status": "pending_c2b_confirmation",
            "evidence_status": "pending_c2b_confirmation",
            "full_component_eligible": False, "routable": False,
        })
    return output
