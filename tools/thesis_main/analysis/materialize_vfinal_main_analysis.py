...
T1_PAIR_FIELDS = [
    "analysis_unit_pair_id", "image_id", "building_id", "block_id", "risk_assist", "pair_analysis_disposition", "source_pair_id",
...
T1_IMAGE_FIELDS = [
    "image_id", "building_id", "block_id", "risk_assist", "n_manual", "n_semi", "n_unique_workers",
    "delivery_adjusted_quality_manual", "delivery_adjusted_quality_semi", "delivery_adjusted_quality_diff_semi_minus_manual",
    "structurally_valid_manual", "structurally_valid_semi", "structurally_valid_diff_semi_minus_manual",
    "valid_only_iou_manual", "valid_only_iou_semi", "valid_only_iou_diff_semi_minus_manual",
    "owner_valid_active_time_manual", "owner_valid_active_time_semi", "owner_valid_active_time_diff_semi_minus_manual",
]
T1_SUMMARY_FIELDS = [
...
V1_SUMMARY_FIELDS = [
    "policy_arm", "risk_route", "n_itt", "resolved_rate", "non_delivery_rate", "unresolved_rate", "severe_failure_rate",
...
V1_STANDARDIZED_FIELDS = [
    ...
    "weight_source_sha256", "resolved_rate", "non_delivery_rate", "unresolved_rate", "severe_failure_rate",
    "delivery_adjusted_quality_mean", "resolved_only_quality_mean", "k_used_mean",
    "active_time_seconds_mean", "completion_time_seconds_mean", "policy_failure_rate",
]
...
def analyze_t1(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    ...
    for pair_id, members in grouped.items():
        ...
        if time_m is not None and time_s is not None:
            active_eligible += 1
        pair_rows.append({
            "analysis_unit_pair_id": pair_id, "image_id": _text(manual.get("image_id")),
            "building_id": _text(manual.get("building_id") or semi.get("building_id")),
            "block_id": _text(manual.get("block_id") or semi.get("block_id")), "risk_assist": risk,
...
def analyze_t1_images(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    """Build the one-image/one-row primary table from resolver-finalized rows."""
    grouped: dict[str, list[dict[str, Any]]] = defaultdict(list)
    disposition_counts: Counter[str] = Counter()
    for row in rows:
        image_id = _text(row.get("image_id"))
        disposition = _text(row.get("pair_analysis_disposition"))
        if not image_id or disposition not in {"included", "administrative_censor", "not_evaluable"}:
            raise ValueError("T1 image analysis requires image_id and resolver-finalized pair disposition")
    ...
    for image_id, members in sorted(grouped.items()):
        conditions = Counter(_text(row.get("condition")).lower() for row in members)
        workers = [_text(row.get("worker_id")) for row in members]
        if len(members) != 4 or conditions != {"manual": 2, "semi": 2}:
            raise ValueError(f"T1 image {image_id} must resolve to exactly 2 Manual + 2 Semi")
        if not all(workers) or len(set(workers)) != 4:
            raise ValueError(f"T1 image {image_id} must use four distinct workers")
        risks = {_risk(row.get("risk_assist"), stage="T1") for row in members}
        if len(risks) != 1:
            raise ValueError(f"T1 image {image_id} has inconsistent risk_assist")
        by_condition = {condition: [row for row in members if _text(row.get("condition")).lower() == condition] for condition in ("manual", "semi")}
        def mean_field(condition: str, field: str, *, required: bool = False) -> float | str:
            values = [_number(row.get(field), field=field, required=required) for row in by_condition[condition]]
            return _mean(values)
        da = {condition: mean_field(condition, "delivery_adjusted_quality", required=True) for condition in by_condition}
        structural = {condition: fmean(float(_bool(row.get("structurally_valid"), field="structurally_valid")) for row in by_condition[condition]) for condition in by_condition}
        valid_iou = {condition: _mean([_number(row.get("iou_to_gt"), field="iou_to_gt") if _bool(row.get("structurally_valid"), field="structurally_valid") else None for row in by_condition[condition]]) for condition in by_condition}
        active = {condition: _mean([_owner_valid_time(row) for row in by_condition[condition]]) for condition in by_condition}
        diff = lambda values: "" if values["manual"] == "" or values["semi"] == "" else values["semi"] - values["manual"]
        output.append({
            "image_id": image_id, "building_id": _text(members[0].get("building_id")),
            "block_id": _text(members[0].get("block_id")), "risk_assist": risks.pop(),
            "n_manual": 2, "n_semi": 2, "n_unique_workers": 4,
            "delivery_adjusted_quality_manual": da["manual"], "delivery_adjusted_quality_semi": da["semi"],
            "delivery_adjusted_quality_diff_semi_minus_manual": diff(da),
            "structurally_valid_manual": structural["manual"], "structurally_valid_semi": structural["semi"],
            "structurally_valid_diff_semi_minus_manual": diff(structural),
            "valid_only_iou_manual": valid_iou["manual"], "valid_only_iou_semi": valid_iou["semi"],
            "valid_only_iou_diff_semi_minus_manual": diff(valid_iou),
            "owner_valid_active_time_manual": active["manual"], "owner_valid_active_time_semi": active["semi"],
            "owner_valid_active_time_diff_semi_minus_manual": diff(active),
        })
    return output, {"n_primary_images": len(output), "pair_disposition_row_counts": dict(sorted(disposition_counts.items()))}
...
def _v1_group(rows: list[dict[str, Any]], arm: str, risk: str) -> dict[str, Any]:
    ...
    return {
        ...
        "resolved_rate": _mean([float(row["_terminal"] == "resolved") for row in subset]),
        "non_delivery_rate": _mean([float(row["_terminal"] != "resolved") for row in subset]),
    ...
    for arm in ("strong_global", "full_integrated"):
        ...
        for name, ordinary_weight, stress_weight, source_sha in [("design_50_50", .5, .5, ""), *weight_sets]:
            ...
            for metric in metrics:
                ...
                if left == "" or right == "":
                    result[metric] = ""
                elif metric == "resolved_only_quality_mean":
                    denominator = ordinary_weight * ordinary["resolved_rate"] + stress_weight * stress["resolved_rate"]
                    result[metric] = "" if not denominator else (
                        ordinary_weight * ordinary["resolved_rate"] * left
                        + stress_weight * stress["resolved_rate"] * right
                    ) / denominator
                else:
                    result[metric] = ordinary_weight * left + stress_weight * right
...
def main() -> None:
    ...
    if args.stage == "T1":
        ...
        images, image_audit = analyze_t1_images(rows)
        audit.update(image_audit)
        outputs = [("t1_pair_analysis.csv", units, T1_PAIR_FIELDS), ("t1_image_primary.csv", images, T1_IMAGE_FIELDS), ("t1_summary.csv", summary, T1_SUMMARY_FIELDS)]
    else:
