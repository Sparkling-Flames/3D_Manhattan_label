...
if(typeof document!=='undefined')(()=>{
  ...
  function card(members,title){
    ...
    members.forEach((i,j)=>{
      ...
      const b=document.createElement('button');b.textContent=v.geometry?'原图／历史3D':'原图／当前点来源';b.onclick=()=>{$('variant-select').value=i;chooseVariant(i,true);$('panorama-panel').open=true;document.querySelector('.workspace').scrollIntoView({behavior:'smooth'});};label.append(b);controls.append(label);
    });
  ...
  function focus(){const r=dataset.cases[currentCase].followup,p=r.pairs_detail[Number($('rr-pair').value)||0];$('rr-focus').replaceChildren(card(p.ids.map(cid=>r.ids.indexOf(cid)),'固定主方案 9°：'+p.kind+' · '+p.workers.join(' / ')+' · '+p.distance.toFixed(3)+'°（不随下方分区对照改变）'));}
  function clusters(){
  ...
  function render(){
    ...
    $('rr-evidence').textContent=JSON.stringify({点号说明:r.point_numbering,AI:r.ai,既有用户原文:r.user_original.length?r.user_original:'指定运行输入中没有匹配原文',指定点:r.point_focus||null},null,2);
    $('rr-pair').replaceChildren(...r.pairs_detail.map((p,i)=>opt(i,p.kind+'：'+p.workers.join(' / '))));
  ...
  $('rr-method').onchange=clusters;$('rr-pair').onchange=focus;
  for(const id of ['rr-relation','rr-defer','rr-comment'])$(id).addEventListener('change',save);$('rr-comment').addEventListener('input',save);
  $('rr-export').onclick=()=>{const blob=new Blob([JSON.stringify({...envelope(),exported_at:new Date().toISOString()},null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='16图分簇_我的审核.json';a.click();URL.revokeObjectURL(url);};
