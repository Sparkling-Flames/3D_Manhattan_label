...
- Crowd modes use complete-link edges only when both similarity channels reach `{data['geometry_cluster']['similarity_cutoff']}` and pointwise topology is compatible; `{data['geometry_cluster']['sensitivity_cutoffs']}` are sensitivity-only. `R_peer_task` remains continuous, and worker bootstrap resamples tasks using the task-equal median.
