...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', default=None,
                        help='Path to a checkpoint (latest.pth or epXXX.pth) to resume from')
    ...
    if args.resume:
        ...
        # 兼容纯 state_dict 和完整 checkpoint
        if 'net' in checkpoint:
            net.load_state_dict(checkpoint['net'])
        else:
            # 假设是纯 state_dict (如官方 ep300.pth)
            print('Warning: Loading pure state_dict. Optimizer/Scheduler will be reset.')
        ...
        if 'optim' in checkpoint:
            ...
            print('Warning: No optimizer state found. Training with fresh optimizer.')

        if 'sched' in checkpoint:
            scheduler.load_state_dict(checkpoint['sched'])
        
        start_epoch = checkpoint.get('epoch', 0) + 1
        if start_epoch > config.training.epoch:
            print('[Warn] Checkpoint epoch exceeds target epoch. Adjust training.epoch or ignore resume.')
    ...
    def save_checkpoint(epoch_idx):
        ...
        ckpt_path = os.path.join(exp_ckpt_root, f'ep{epoch_idx}.pth')
        torch.save(payload, ckpt_path)
        torch.save(payload, os.path.join(exp_ckpt_root, 'latest.pth'))
        print(f'Model checkpoint saved at epoch {epoch_idx}')

    # Start training
    for iep in trange(start_epoch, config.training.epoch + 1, position=0):
    ...
    # Ensure final checkpoint exists even if epoch count not divisible by save_every
    last_epoch = config.training.epoch
    if (last_epoch % config.training.save_every) != 0 and last_epoch >= start_epoch:
